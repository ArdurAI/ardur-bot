...
[schema 3](../packages/testkit/src/performance-report.ts), with its complete
...
[`startScoreboardTrace`](../packages/adapters/src/scoreboard-trace.ts):
...
The new [`collectTraceEvidence`](../packages/testkit/src/scoreboard/trace-collector.ts) accepts
...
[`sendThreadMessage`](../apps/api/src/thread-target.ts) receipt, queue job, executor, durable events
...
[`GraphileJobPublisher.enqueue`](../packages/adapters/src/wakeup.ts) records submission and
...
For supported Pi HTTP routes, [`observePiUsage`](../packages/adapters/src/pi-request-usage.ts) uses
...
[`finalizeRun`](../packages/db/src/events.ts) runs after a successful transaction and before realtime
...
and drift allowance. [`calibrateTraceClock`](../packages/testkit/src/scoreboard/trace-collector.ts)
...
[`RPCLink`](../apps/web/src/lib/rpc.ts). Normal startup does not import the collector. A failed
