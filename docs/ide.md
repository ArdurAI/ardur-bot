...
`apps/web/src/pages/ide/editor.tsx` owns every CodeMirror import. `App.tsx` imports the IDE page lazily. The page imports its editor so that the editor is ready before a file read starts. The Vite group `ide-codemirror` keeps CodeMirror and its parser dependencies in one lazy chunk. `ide-imports.test.ts` checks the static import graph. The browser test checks that the Bots page does not fetch that chunk.
...
The supported navigation guard requires the data router in `main.tsx`. Report initial-JavaScript growth against both the untouched worktree and `docs/performance/bundle-baseline.json`; the 10 KiB budget is advisory. Do not refresh that baseline just to hide growth. CodeMirror is measured separately as a lazy chunk.
