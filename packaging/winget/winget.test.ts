...
describe("winget manifests", () => {
  it("have valid offline schema fields", async () => {
    const installer = yaml.load(await readFile(new URL("./ArdurAI.ArdurBot.installer.yaml", import.meta.url), "utf8"));
    const locale = yaml.load(await readFile(new URL("./ArdurAI.ArdurBot.locale.en-US.yaml", import.meta.url), "utf8"));
    const version = yaml.load(await readFile(new URL("./ArdurAI.ArdurBot.yaml", import.meta.url), "utf8"));
