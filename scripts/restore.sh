...
until "${compose[@]}" exec -T postgres pg_isready -U ardurbot >/dev/null 2>&1; do
  sleep 1
done
...
"${compose[@]}" exec -T postgres psql -X -U ardurbot -d ardurbot \
  --set=ON_ERROR_STOP=on --single-transaction --file=- < "$SRC/ardurbot.sql"
