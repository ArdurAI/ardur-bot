...
import type { AgentRunRequest, AgentRuntimeEvent, ConnectorTool } from "@ardurbot/adapter-kit";
import { PiAgentRuntime } from "@ardurbot/adapters";
...
describe("real Pi against an offline model HTTP endpoint", () => {
  it("assembles fragmented tool arguments, executes the write, and sends its result back", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "ardurbot-pi-offline-"));
