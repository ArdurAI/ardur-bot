...
function fixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "ardurbot-backup-"));
  ...
  for (const script of [
    ...
    if (script.endsWith("backup-prod.sh")) {
      ...
      expect(source).toContain('BACKUP_ROOT="/var/backups/ardurbot"');
      source = source.replace('BACKUP_ROOT="/var/backups/ardurbot"', `BACKUP_ROOT="${snapshots}"`);
    }
...
describe("development backup failures", () => {
  it.each([false, true])("archives data with a root env file present: %s", (hasEnv) => {
    ...
    expect(readFileSync(path.join(output, "ardurbot.sql"), "utf8")).toContain("CREATE TABLE");
  ...
  it.each(["absent", "skipped"])("allows an empty archive when data is %s", (scenario) => {
    ...
    const result = f.run("scripts/backup.sh", ["example"], {
      ARDURBOT_BACKUP_SKIP_HOMES: scenario === "skipped" ? "1" : "0",
    });
...
describe("production backup deployment and archive behavior", () => {
  it.each([false, true])("uses the configured deployment directory: %s", (custom) => {
    const f = fixture();
    const deployment = custom ? f.checkout : "/srv/ardurbot";
    const result = f.run(
      "infra/compose/backup-prod.sh",
      [],
      custom ? { ARDURBOT_DEPLOY_DIR: deployment } : {},
    );
    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout).toContain("Verified Ardur Bot backup");
  ...
  it("provides optional systemd configuration for installed copies", () => {
    const service = readFileSync(
      path.join(repoRoot, "infra/systemd/ardurbot-backup.service"),
      "utf8",
    );
    expect(service).toContain("EnvironmentFile=-/etc/ardurbot/backup.env");
  });

  it("rejects relative deployment paths before any backup commands", () => {
    const f = fixture();
    const result = f.run("infra/compose/backup-prod.sh", [], { ARDURBOT_DEPLOY_DIR: "relative" });
    expect(result.status).not.toBe(0);
  ...
  it.each([{ FAIL_TAR: "2" }, { FAIL_DUMP: "1" }, { FAIL_VERIFY: "1" }])(
    ...
    (failure) => {
      ...
      expect(result.status).not.toBe(0);
      expect(result.stdout).not.toContain("Verified Ardur Bot backup");
    },
  );
});
