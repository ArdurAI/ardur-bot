...
import type { RunsListOutput, ThreadSnapshot } from "@ardurbot/contracts";
import { isTerminal } from "@ardurbot/core";
...
const project = `ardurbot-topology-${process.pid}-${Date.now().toString(36)}`;
...
async function main() {
  ...
  const env = {
    ...
    TOPOLOGY_APP_IMAGE: process.env.TOPOLOGY_APP_IMAGE ?? "ardurbot/app:topology",
    TOPOLOGY_COMPUTER_IMAGE: process.env.TOPOLOGY_COMPUTER_IMAGE ?? "ardurbot/computer:topology",
  };
...
async function waitForWorker(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    // {"timestamp":...,"level":"info","message":"worker ready","service.name":"ardurbot-worker"},
    // not the old plain "ardurbot worker ready" console.log line -- match the JSON field directly
...
function assertComputerNetworkIsolation(
  ...
  const managed = docker([
    ...
    computerId,
    "--format",
    '{{index .Config.Labels "ardurbot.managed"}}',
  ]).trim();
