...
async function main() {
  ...
  if (unknown.length)
    throw new Error(
      "Usage: computer-replay.ts [--image=ardurbot/computer:local] [--subnet=unused-private-CIDR] [--live --record=new-fixture.json]",
    );
  const image = imageArg?.slice("--image=".length) || "ardurbot/computer:local";
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "ardurbot-computer-replay-"));
