...
function performanceEnvironment(databaseUrl: string): NodeJS.ProcessEnv {
  return {
    ...
    E2B_API_KEY: "",
    DAYTONA_API_KEY: "",
    BETTER_AUTH_SECRET: "ardurbot-benchmark-auth-secret-over-32-characters",
    ENCRYPTION_KEY: "ardurbot-benchmark-encryption-key-over-32-characters",
    SANDBOX_SUPERVISOR_TOKEN: "ardurbot-benchmark-supervisor-token-over-32-characters",
    SCREEN_PROXY_SECRET: "ardurbot-benchmark-screen-proxy-secret-over-32-characters",
    ...
    ARDURBOT_HOST: "127.0.0.1",
    ARDURBOT_WEB_URL: webOrigin,
    ARDURBOT_DISABLE_BUNDLED_RENDERER: remoteRenderer ? "1" : "0",
    ARDURBOT_DISABLE_WARM_WINDOW: disableWarmWindow ? "1" : "0",
    ARDURBOT_PERFORMANCE_ASSET_DELAY_MS: String(assetDelayMs),
...
function buildProductionArtifacts(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@ardurbot/desktop", "pack:dir"], env);
}

function migrateDatabase(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@ardurbot/db", "generate"], env);
  run("pnpm", ["--filter", "@ardurbot/db", "exec", "prisma", "migrate", "deploy"], env);
}
...
async function launchDesktop(
  ...
  const app = await electron.launch({
    ...
    env: {
      ...benchmark.env,
      ARDURBOT_PERFORMANCE_USER_DATA: profile,
      ARDURBOT_PERFORMANCE_CLEAR_CACHE: clearCache ? "1" : "0",
    },
  });
