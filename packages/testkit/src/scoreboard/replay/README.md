...
The tapes include the eight Board schemas from the ordinary built-in tool catalog. Their addition was checked against a current production recording: removing those schemas produces the previous task-23 tape exactly. The same schema addition applies to tool-bearing requests in all 30 tapes; prompts, responses, declared variables and existing schemas are preserved. Tool availability does not grant task permission: the grader still checks calls against each task's allowed tools.
...
`OutcomeObservation.reply` is required. `runProductionTask` collects persisted bot text for the current run, including durable progress messages and the final reply. Redaction checks both that text and the saved result; a correct artifact cannot excuse a reply that discloses the private synthetic sentinel. Missing reply observations fail the redaction check. Reports retain the observation hash and check results, without copying the reply text.
...
The monotonic deadline is checked before and after counting, immediately before transport dispatch, and before accepting provider or runner completion. The timer still aborts pending asynchronous work. Synchronous work cannot dispatch or succeed after the deadline merely because it delayed the timer callback.
...
The runner writes its binding before trials and each task result before cleanup. Trial cleanup runs in a nested `finally` even if report persistence fails; the write error remains a failed invocation. These files preserve partial evidence if a later task or process fails. Graphile acknowledgement and immediately runnable compaction jobs are drained before the worker pool closes; run completion alone does not prove the queue finished its bookkeeping.
