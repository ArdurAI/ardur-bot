...
function postgresUp() {
  try {
    execSync(`docker compose -f ${composeFile} exec -T postgres pg_isready -U ardurbot`, {
      stdio: "ignore",
      timeout: 8_000,
    });
    return true;
...
describeBackup("compose backup and restore", () => {
  it("dumps postgres and restores into a side database", () => {
    ...
    execSync(`${backupScript} ${stamp}`, {
      stdio: "pipe",
      timeout: 60_000,
      env: { ...process.env, ARDURBOT_BACKUP_SKIP_HOMES: "1" },
    });
    const dump = path.resolve("backups", stamp, "ardurbot.sql");
    ...
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ardurbot -d postgres -c "DROP DATABASE IF EXISTS ardurbot_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ardurbot -d postgres -c "CREATE DATABASE ardurbot_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ardurbot -d ardurbot_restore_test`,
      {
        input: sql,
    ...
    const tables = execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ardurbot -d ardurbot_restore_test -c "\\dt"`,
      { encoding: "utf8", timeout: 20_000 },
    );
    expect(tables).toMatch(/bots/);
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ardurbot -d postgres -c "DROP DATABASE ardurbot_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
