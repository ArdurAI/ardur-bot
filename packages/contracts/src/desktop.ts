...
export interface ArdurBotDesktopUpdate {
...
export interface ArdurBotDesktopOAuthCallback {
  code: string;
  state?: string;
}
...
export interface ArdurBotDesktop {
  ...
  oauth: {
     ...
     */
    onCallback: (listener: (callback: ArdurBotDesktopOAuthCallback) => void) => () => void;
  };
}
