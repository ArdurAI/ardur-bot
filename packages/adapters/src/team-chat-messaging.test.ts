import type { MessagingInboundMessage, MessagingSurface } from "@ardurbot/adapter-kit";
...
describe("toTeamChatInbound", () => {
  ...
  it("treats @mentions as mention when kind is omitted", () => {
    expect(toTeamChatInbound(baseEvent({ content: "hey @ardurbot look" }))?.kind).toBe("mention");
  });
});
