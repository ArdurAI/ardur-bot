...
describe("executor approval replay", () => {
  ...
  it("replays an approved lazy tool through the exposed catalog executor", () => {
    ...
    expect(continuation).not.toContain("__ardurbotCatalogTool");
  });
  ...
  it("keeps a direct-tool argument named like the catalog marker in continuation JSON", () => {
      ...
      [
        {
          ...
          request: {
            ...
            __ardurbotCatalogTool: "installed_execute_tool",
          },
        },
      ],
      JSON.stringify,
    ...
    expect(continuation).toContain(
      'notes.write: {"id":"row-1","arguments":{"mode":"strict"},"text":"approved exactly","__ardurbotCatalogTool":"installed_execute_tool"}',
    );
  });
  ...
  it.each([
    ...
    const queue = createApprovedEffectReplayQueue([{ kind: "mcp__demo__send_message", request }]);
    expect(approvedCatalogReplay(queue, called, "__ardurbotCatalogTool", true).error).toContain(
      "must be replayed before",
    );
    expect(queue.nextRequest()).toBe(request);
    expect(
      approvedCatalogReplay(queue, "connectors_execute_tool", "__ardurbotCatalogTool", false),
    ).toEqual({});
  });
  ...
  it("preserves direct approved args that use the catalog marker as data", () => {
    const approvedRequest = {
      __ardurbotCatalogTool: "user-provided-value",
      target: "approved",
    };
    ...
    expect(
      approvedReplayArgs(
        approvedRequest,
        { __ardurbotCatalogTool: "user-provided-value", target: "reconstructed" },
        "__ardurbotCatalogTool",
      ),
