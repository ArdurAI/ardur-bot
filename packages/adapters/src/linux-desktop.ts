...
export class LinuxDesktop {
  ...
  private async ensure(computer: ComputerRef, context: AdapterContext) {
    ...
    const match = output.match(/ARDURBOT_DESKTOP=(\d+):([a-zA-Z0-9_-]+)/);
...
function browserActionCommand(
  ...
  if (browser !== undefined) {
    return `nohup ${browserLauncherPath(layout.displayNumber)} ${shellQuote(browser)} </dev/null >/tmp/ardurbot/browser-open-${layout.displayNumber}.log 2>&1 &`;
  }
