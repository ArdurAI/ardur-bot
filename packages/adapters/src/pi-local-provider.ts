...
export const LOCAL_VISION_MODELS_ENV = "ARDURBOT_LOCAL_VISION_MODELS";
...
export function localBaseUrl(): string {
  const value = process.env.ARDURBOT_LOCAL_MODELS_URL?.trim() || DEFAULT_BASE_URL;
  let url: URL;
  try {
    url = new URL(value);
  } catch {
    throw new Error("ARDURBOT_LOCAL_MODELS_URL must be an absolute HTTP(S) URL");
  }
...
function localModelIds(): string[] {
  return (process.env.ARDURBOT_LOCAL_MODELS ?? "")
    .split(",")
    .map((id) => id.trim())
...
function localModel(id: string): Model<"openai-completions"> {
  return {
    ...
    contextWindow: tokenLimit("ARDURBOT_LOCAL_CONTEXT_WINDOW", DEFAULT_CONTEXT_WINDOW),
    maxTokens: tokenLimit("ARDURBOT_LOCAL_MAX_TOKENS", DEFAULT_MAX_TOKENS),
  };
}
