...
describe("Pi JSONL sessions", () => {
  ...
  it("uses Pi's session format for context and completed messages", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ardurbot-pi-session-"));
    try {
      ...
      expect(records).toContainEqual(
        expect.objectContaining({
          kind: "entry",
          customType: "ardurbot_context",
          data: expect.objectContaining({
            ardurbotThreadId: "thread-1",
            ardurbotTraceId: "trace-1",
          }),
        }),
      );
  ...
  it("keeps sessions scoped and bounded by the retention policy", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ardurbot-pi-retention-"));
    try {
  ...
  it("removes bot and account session scopes", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ardurbot-pi-cleanup-"));
    try {
