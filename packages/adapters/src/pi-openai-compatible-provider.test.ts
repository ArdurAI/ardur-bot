...
describe("model connect", () => {
  ...
  it("rejects public http openai-compatible connects when an API key is set", () => {
    const previous = process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC;
    process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC = "1";
    try {
      expect(() =>
        prepareOpenAiCompatibleConnect({
          provider: OPENAI_COMPATIBLE_PROVIDER_ID,
      ...
      if (previous === undefined) delete process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC;
      else process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC = previous;
    }
  });
...
describe("openai-compatible provider", () => {
  ...
  it("lists openai-compatible in the catalog even without ARDURBOT_LOCAL_MODELS", () => {
    delete process.env.ARDURBOT_LOCAL_MODELS;
