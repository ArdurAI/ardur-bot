...
export function openAiCompatAllowPublicHosts(): boolean {
  return process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC === "1";
}
...
export function assertAllowedOpenAiCompatibleRequestUrl(
  ...
  if (!allowPublic) {
    throw new Error(
      "Public model endpoints are blocked. Set ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC=1 to allow them. A private reverse proxy on localhost or an RFC1918 address does not need that gate.",
    );
  }
  return url;
}
