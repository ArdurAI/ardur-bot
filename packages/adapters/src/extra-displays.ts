import type { ComputerAction, ComputerInput } from "@ardurbot/adapter-kit";
...
export function observeExtraDisplayCommand(layout: ExtraDisplayLayout): string {
  const imagePath = `/tmp/ardurbot/observe-${layout.displayNumber}.png`;
  return [
    `DISPLAY=${layout.display} xdotool getmouselocation --shell >/tmp/ardurbot/cursor-${layout.displayNumber}.txt || true`,
    ...
    `printf '\\nCURSOR '`,
    `tr '\\n' ' ' </tmp/ardurbot/cursor-${layout.displayNumber}.txt 2>/dev/null || true`,
  ].join("; ");
}
