import type { AdapterContext, MessagingInboundEvent } from "@ardurbot/adapter-kit";
...
function webhookRequest(payload: unknown): Request {
  return new Request("https://ardurbot.test/api/v1/messaging/webhook/mock", {
    method: "POST",
...
describe("ChatSdkMessagingSurface webhook plumbing", () => {
  ...
  it("rejects oversized bodies before they reach the adapter", async () => {
    ...
    const response = await surface.handleWebhook(
      "mock",
      new Request("https://ardurbot.test/api/v1/messaging/webhook/mock", {
        method: "POST",
