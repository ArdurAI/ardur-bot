 ...
 * This is deliberately a recorder only: Ardur Bot remains responsible for running
...
export class PiJsonlSessionRecorder implements PiSessionRecorder {
  ...
  async start(input: PiSessionStart): Promise<PiSessionHandle> {
    ...
    await handle.appendCustomEntry("ardurbot_context", {
      ardurbotUserId: input.userId,
      ardurbotBotId: input.botId,
      ardurbotRunId: input.runId,
      ardurbotThreadId: input.threadId,
      ...(input.traceId ? { ardurbotTraceId: input.traceId } : {}),
      model: input.model,
