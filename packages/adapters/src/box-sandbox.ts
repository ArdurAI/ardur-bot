...
const BOX_WORKSPACE = "/home/user/ardurbot-home";
...
export class BoxSandboxProvider implements SandboxProvider {
    ...
    const created = await this.client.create(
      {
        createBoxRequest: {
          ...
          noEnv: true,
          env: {
            ARDURBOT_BOT_ID: request.botId,
            ARDURBOT_SANDBOX: "computer",
          },
        },
      },
      { signal: context.signal },
  ...
  private async runCommand(
    ...
    const marker = `/tmp/ardurbot-command-${randomUUID()}.completed-124`;
  ...
  private async terminateCommand(id: string, marker: string): Promise<void> {
    const pattern = marker.replace("ardurbot", "[r]akazo");
    await this.rawCommand(
      id,
...
function boxCwd(cwd: string | undefined): string {
  if (
    ...
    return "ardurbot-home";
  }
  const relative = cwd.startsWith(`${BOX_WORKSPACE}/`)
    ? cwd.slice(BOX_WORKSPACE.length + 1)
    : cwd.startsWith("/home/ardurbot/")
      ? cwd.slice("/home/ardurbot/".length)
      : cwd;
  return path.posix.join("ardurbot-home", normalizeWorkspacePath(relative));
}
