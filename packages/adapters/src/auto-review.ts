...
import type { AutoReviewJudgeDecision } from "@ardurbot/core";
...
function localModelIds(env: NodeJS.ProcessEnv): string[] {
  return (env.ARDURBOT_LOCAL_MODELS ?? "")
    .split(",")
    .map((id) => id.trim())
...
export function deploymentAutoReviewDefault(env: NodeJS.ProcessEnv = process.env): boolean {
  return envFlag(env, "ARDURBOT_AUTO_REVIEW");
}

export function autoReviewTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.ARDURBOT_AUTO_REVIEW_TIMEOUT_MS?.trim();
...
export function autoReviewMinConfidence(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.ARDURBOT_AUTO_REVIEW_MIN_CONFIDENCE?.trim();
...
/**
 * `ARDURBOT_AUTO_REVIEW_PROVIDER=jev` selects TypeSafe Jev when a key is present;
...
export function resolveAutoReviewProviderKind(
  ...
  const requested = env.ARDURBOT_AUTO_REVIEW_PROVIDER?.trim().toLowerCase();
...
export function resolveAutoReviewChecker(
  ...
  if (kind === "jev") {
    return {
      ...
      model: env.ARDURBOT_AUTO_REVIEW_MODEL?.trim() || DEFAULT_JEV_MODEL,
    };
  }
  if (kind === "scripted") {
  ...
  const overrideProvider = env.ARDURBOT_AUTO_REVIEW_PROVIDER?.trim();
  const overrideModel = env.ARDURBOT_AUTO_REVIEW_MODEL?.trim();
