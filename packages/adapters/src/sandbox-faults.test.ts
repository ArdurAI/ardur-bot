...
import type { PortableFile, SandboxProvider } from "@ardurbot/adapter-kit";
...
describe.each([
  ...
  [
    "desktop",
    async () => {
      const root = await mkdtemp(path.join(tmpdir(), "ardurbot-sandbox-faults-"));
...
async function providerSet(label: string): Promise<Array<[string, SandboxProvider]>> {
  const root = await mkdtemp(path.join(tmpdir(), `ardurbot-sandbox-transfer-${label}-`));
  temporaryRoots.push(root);
  return [
