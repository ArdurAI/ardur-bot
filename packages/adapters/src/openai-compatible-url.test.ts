...
const savedAllowPublic = process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC;

afterEach(() => {
  if (savedAllowPublic === undefined) delete process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC;
  else process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC = savedAllowPublic;
});
...
describe("openai-compatible URL policy", () => {
  ...
  it("rejects public hosts unless explicitly allowed", () => {
    delete process.env.ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC;
