...
const ENV_KEYS = [
  "ARDURBOT_LOCAL_MODELS",
  "ARDURBOT_LOCAL_MODELS_URL",
  "ARDURBOT_LOCAL_CONTEXT_WINDOW",
  "ARDURBOT_LOCAL_MAX_TOKENS",
] as const;
...
function setModels(value: string | undefined) {
  if (value === undefined) delete process.env.ARDURBOT_LOCAL_MODELS;
  else process.env.ARDURBOT_LOCAL_MODELS = value;
}
...
describe("local model provider", () => {
  ...
  it("defaults to the Ollama endpoint and honours an override", () => {
    delete process.env.ARDURBOT_LOCAL_MODELS_URL;
    expect(localBaseUrl()).toBe("http://127.0.0.1:11434/v1");

    process.env.ARDURBOT_LOCAL_MODELS_URL = "http://127.0.0.1:1234/v1";
  ...
  it("rejects an invalid or non-HTTP endpoint", () => {
    for (const bad of ["not-a-url", "file:///tmp/model.sock", "ftp://127.0.0.1/models"]) {
      process.env.ARDURBOT_LOCAL_MODELS_URL = bad;
      expect(() => localBaseUrl(), `endpoint "${bad}"`).toThrow(
        /ARDURBOT_LOCAL_MODELS_URL must be an absolute HTTP\(S\) URL/,
      );
    }
  });
  ...
  it("defaults token limits when unset, and honours valid overrides", () => {
    setModels("qwen3:4b");
    delete process.env.ARDURBOT_LOCAL_CONTEXT_WINDOW;
    delete process.env.ARDURBOT_LOCAL_MAX_TOKENS;
    let model = localProvider()?.getModels()[0];
    expect(model?.contextWindow).toBe(32_768);
    expect(model?.maxTokens).toBe(4_096);

    process.env.ARDURBOT_LOCAL_CONTEXT_WINDOW = "8192";
    process.env.ARDURBOT_LOCAL_MAX_TOKENS = "512";
  ...
  it("rejects token limits that are not positive integers", () => {
    ...
    for (const bad of ["abc", "-5", "0", "3.7", "Infinity", "1e400"]) {
      process.env.ARDURBOT_LOCAL_CONTEXT_WINDOW = bad;
      expect(() => localProvider(), `context window "${bad}"`).toThrow(
        /ARDURBOT_LOCAL_CONTEXT_WINDOW must be a positive integer/,
      );
    }
    delete process.env.ARDURBOT_LOCAL_CONTEXT_WINDOW;

    process.env.ARDURBOT_LOCAL_MAX_TOKENS = "-1";
    expect(() => localProvider()).toThrow(/ARDURBOT_LOCAL_MAX_TOKENS must be a positive integer/);
  });
