...
describe("computer provisioning", () => {
  it("stops a provider when archive invalidates its boot claim", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-provision-race-"));
  ...
  it("reclaims a stale suspending computer after the wait times out", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-stale-suspend-reclaim-"));
  ...
  it("does not adopt a concurrent reclaim stamp after claiming", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-claim-stamp-race-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
  ...
  it("advances the claim stamp past the observed updatedAt when the clock does not move", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-claim-stamp-skew-"));
    const observed = new Date("2024-01-01T00:00:00.000Z");
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
  ...
  it("does not reclaim a fresh booting claim without waiting", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-fresh-boot-claim-"));
    const nowMs = Date.parse("2024-06-01T12:00:00.000Z");
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
  ...
  it("does not reclaim a stale booting claim while another run still holds a live worker lease", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-stale-boot-live-run-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
  ...
  it("activates a boot even if another Team bot takes a lease mid-provision", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-team-lease-mid-boot-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
  ...
  it.each([
    ...
    async ({ stage, rollbackFails }) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-reconnect-rollback-"));
  ...
  it("does not stop a pre-existing computer when reconnect setup fails", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-reconnect-unowned-"));
  ...
  it.each([
    ...
    async (next) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-provision-reconnect-update-"));
  ...
  it.each([
    ...
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-prepare-rollback-"));
    const ref = {
      id: "provider-1",
      botId: "bot-1",
      kind: "fake" as const,
  ...
  it.each([false, true])(
    ...
    async (prepareFails) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-provision-reconnect-"));
      const ref = {
        id: "provider-1",
        botId: "bot-1",
        kind: "cloud" as const,
...
describe("computer replacement", () => {
  ...
  it("replaces a wedged computer and restores the durable home", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-replace-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "ardurbot-replace-home-"));
  ...
  it("claims a stopped computer before teardown so concurrent replacements serialize", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-replace-stopped-claim-"));
    const updateMany = vi
      .fn()
      .mockResolvedValueOnce({ count: 1 })
      .mockResolvedValueOnce({ count: 1 })
  ...
  it("continues recover when checkpoint fails with an ordinary provider error", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-recover-checkpoint-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "ardurbot-recover-checkpoint-home-"));
    const home = new LocalAgentHomeStore(homeRoot);
    const sandbox = new FakeSandboxProvider();
    const first = await sandbox.provision({ botId: "bot-1", homePath: dataDir }, context);
  ...
  it("aborts update when checkpoint fails with an ordinary provider error", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "ardurbot-update-checkpoint-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "ardurbot-update-checkpoint-home-"));
    const home = new LocalAgentHomeStore(homeRoot);
    const sandbox = new FakeSandboxProvider();
    const first = await sandbox.provision({ botId: "bot-1", homePath: dataDir }, context);
    vi.spyOn(sandbox, "exportWorkspace").mockImplementation(() => {
