...
export async function probeCodex(): Promise<RuntimeAvailability> {
  const base = { runtimeKind: "codex-app-server" as const, models: [] };
  let rpc: CodexRpc | undefined;
  try {
    const binary = await findNativeBinary("codex");
    if (!binary) throw new Error("missing");
    const { version } = await probeCommand(binary, ["--version"], true);
    rpc = await openCodex();
...
export class CodexAppServerRuntime implements AgentRuntime {
  ...
  async *run(
    ...
    const rpc = await openCodex(this.start).catch(() => {
