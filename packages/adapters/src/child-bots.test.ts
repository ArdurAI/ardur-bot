...
import type { createRepos, PrismaClient } from "@ardurbot/db";
...
describe("spawned bot creation", () => {
  ...
  it("passes computerMode through to createBot", async () => {
    ...
    const createReposSpy = vi.spyOn(await import("@ardurbot/db"), "createRepos").mockReturnValue({
      createBot,
      ...
      destroyBot(
        {
          ...
          home: {} as AgentHomeStore,
          jobs: { cancel: vi.fn() } as unknown as JobPublisher,
          dataDir: "/tmp/ardurbot-destroy-bot-test",
        },
        {
          id: "bot-1",
          userId: "user-1",
