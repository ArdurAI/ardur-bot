...
describe("resolveAutoReviewChecker", () => {
  it("prefers explicit env overrides", () => {
    expect(
      resolveAutoReviewChecker({
        ARDURBOT_AUTO_REVIEW_PROVIDER: "openrouter",
        ARDURBOT_AUTO_REVIEW_MODEL: "cheap/fast",
        ARDURBOT_LOCAL_MODELS: "local-a",
  ...
  it("prefers local models when configured", () => {
    expect(
      resolveAutoReviewChecker({
        ARDURBOT_LOCAL_MODELS: " llama-local , other ",
  ...
  it("selects Jev when the provider is jev and a TypeSafe key is present", () => {
    ...
    expect(
      resolveAutoReviewChecker({
        ARDURBOT_AUTO_REVIEW_PROVIDER: "jev",
        ARDURBOT_AUTO_REVIEW_MODEL: "jev-custom",
        TYPESAFE_API_KEY: "ts-key",
      }),
...
describe("resolveAutoReviewProviderKind", () => {
  it("uses Jev only when a TypeSafe key is present", () => {
    expect(resolveAutoReviewProviderKind({ ARDURBOT_AUTO_REVIEW_PROVIDER: "jev" })).toBe("llm");
    expect(
      resolveAutoReviewProviderKind({
        ARDURBOT_AUTO_REVIEW_PROVIDER: "jev",
        TYPESAFE_API_KEY: "ts-key",
    ...
    expect(resolveAutoReviewProviderKind({ ARDURBOT_AUTO_REVIEW_PROVIDER: "openrouter" })).toBe(
      "llm",
    );
    expect(
      resolveAutoReviewProviderKind({
        ARDURBOT_AUTO_REVIEW_PROVIDER: "scripted",
...
describe("isAutoReviewCheckerConfigured", () => {
  it("requires a runnable checker", () => {
    ...
    expect(
      isAutoReviewCheckerConfigured({
        env: { ARDURBOT_LOCAL_MODELS: "local-1" },
      }),
    ).toBe(true);
    expect(
      isAutoReviewCheckerConfigured({
    ...
    expect(
      isAutoReviewCheckerConfigured({
        env: { ARDURBOT_AUTO_REVIEW_PROVIDER: "jev", TYPESAFE_API_KEY: "ts-key" },
      }),
    ).toBe(true);
    expect(
      isAutoReviewCheckerConfigured({
    ...
    expect(
      isAutoReviewCheckerConfigured({
        env: { ARDURBOT_AUTO_REVIEW_PROVIDER: "scripted", AGENT_RUNTIME: "scripted" },
      }),
    ).toBe(true);
    expect(
      isAutoReviewCheckerConfigured({
        env: { ARDURBOT_AUTO_REVIEW_PROVIDER: "scripted" },
      }),
    ).toBe(false);
  });
});
...
describe("deploymentAutoReviewDefault", () => {
  it("defaults off", () => {
    ...
    expect(deploymentAutoReviewDefault({ ARDURBOT_AUTO_REVIEW: "1" })).toBe(true);
    expect(deploymentAutoReviewDefault({ ARDURBOT_AUTO_REVIEW: "true" })).toBe(true);
  });
});
...
describe("autoReviewTimeoutMs", () => {
  it("defaults to 1500 and clamps bad values", () => {
    ...
    expect(autoReviewTimeoutMs({ ARDURBOT_AUTO_REVIEW_TIMEOUT_MS: "2000" })).toBe(2_000);
    expect(autoReviewTimeoutMs({ ARDURBOT_AUTO_REVIEW_TIMEOUT_MS: "nope" })).toBe(1_500);
  });
});
...
describe("autoReviewMinConfidence", () => {
  it("defaults to 0.5 and clamps out of range", () => {
    ...
    expect(autoReviewMinConfidence({ ARDURBOT_AUTO_REVIEW_MIN_CONFIDENCE: "0.8" })).toBe(0.8);
    expect(autoReviewMinConfidence({ ARDURBOT_AUTO_REVIEW_MIN_CONFIDENCE: "2" })).toBe(0.5);
    expect(autoReviewMinConfidence({ ARDURBOT_AUTO_REVIEW_MIN_CONFIDENCE: "nope" })).toBe(0.5);
  });
});
