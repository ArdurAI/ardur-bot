...
describe("Docker sandbox", () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        headers: expect.objectContaining({
          ...
          "x-ardurbot-bot-id": "home-bot",
          "x-ardurbot-screen-id": "bot",
          "x-ardurbot-screen-lease-id": "run-1:1",
  ...
  it("asks the supervisor to cancel orphaned run work when releasing a screen", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "DELETE",
        headers: expect.objectContaining({ "x-ardurbot-cancel-run-work": "1" }),
      }),
    );
  });
});
