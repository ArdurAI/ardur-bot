...
} from "@ardurbot/core/command-limits";
import {
...
import type { Win32FileHandle } from "./desktop-sandbox-win32-path.js";
import {
...
export class DesktopSandboxProvider implements SandboxProvider {
  ...
  async provision(
    ...
    if (this.opts.restricted) {
      if (!/^[a-zA-Z0-9_-]+$/.test(request.botId)) throw new Error("Invalid host computer.");
      // Validate each existing parent before recursive mkdir can follow it.
      const root = await realpath(this.opts.root!);
      for (const parent of [path.dirname(home), home]) {
        const present = await lstat(parent).catch((error) => {
          if (hasErrorCode(error, "ENOENT")) return null;
          throw error;
        });
        if (
          present &&
          (present.isSymbolicLink() || !isAllowedDesktopPath(await realpath(parent), [root]))
        )
  ...
  async destroy(computer: ComputerRef, _context: AdapterContext): Promise<void> {
    ...
    if (box && this.opts.root && !this.opts.restricted) {
...
async function readContainedWorkspaceFile(home: string, target: string, maxBytes: number) {
  ...
  try {
    handle = await open(target, constants.O_RDONLY | O_NOFOLLOW | (constants.O_NONBLOCK ?? 0));
    await assertContainedFileHandle(
      handle,
      resolvedHome,
      parent,
      path.dirname(target),
    ...
    // Read at most the limit plus one even when the file grows after stat.
