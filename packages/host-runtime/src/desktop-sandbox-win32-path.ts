...
/** The subset of the native FFI API used by the contained Windows writer. */
export interface Win32NativeApi {
  load(name: string): { func(prototype: string): KoffiFunction };
  struct(name: string, members: Record<string, string>): unknown;
  sizeof(type: unknown): number;
}

type KoffiFunction = (...args: any[]) => unknown;
let loadNative: (() => Win32NativeApi | undefined) | undefined;

/** Composition roots supply either the installed module or the packaged addon. */
export function installWin32NativeApi(loader: () => Win32NativeApi | undefined) {
  loadNative = loader;
  cached = undefined;
}
...
function nt(): NtFns {
  ...
  if (process.platform !== "win32") throw new Error("Win32 NT APIs unavailable");
  const koffi = loadNative?.();
  if (!koffi) throw new Error("Native directory handles unavailable.");
