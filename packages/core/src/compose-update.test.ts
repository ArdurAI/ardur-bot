...
const target = {
  composeFiles: ["/srv/ardurbot/infra/compose/docker-compose.prod.yml"],
  envFiles: ["/srv/ardurbot/.env"],
};
...
describe("image references", () => {
  ...
  it("accepts registry paths with a host and rejects anything else", () => {
    ...
    expect(isValidImageName("ardurbot/app")).toBe(true);
...
describe("compose argv construction", () => {
  ...
  it("passes one --file per Compose file, in overlay order", () => {
    const overlaid = {
      ...
      composeFiles: [
        ...
        "/srv/ardurbot/ops/compose/docker-compose.sandbox.yml",
      ],
    };
  ...
  it("defaults -p to the name pinned in the compose file", () => {
    expect(DEFAULT_COMPOSE_PROJECT_NAME).toBe("ardurbot-prod");
    expect(composePsArgv(target).args.slice(0, 3)).toEqual(["compose", "-p", "ardurbot-prod"]);
  });
  ...
  it("prefers the name Compose injects into running services", () => {
    ...
    expect(
      resolveComposeProjectName({
        ...
        ARDURBOT_COMPOSE_PROJECT_NAME: "manual-stack",
      }),
    ).toBe("live-stack");
  });

  it("falls back to a dedicated override, then to the name pinned in the compose file", () => {
    expect(resolveComposeProjectName({ ARDURBOT_COMPOSE_PROJECT_NAME: "manual-stack" })).toBe(
      "manual-stack",
    );
...
describe("update plans", () => {
  ...
  it("fetches, fast-forwards, then builds on the fork path", () => {
    const steps = composeUpdatePlan({
      strategy: "build",
      target,
      repoUrl: "https://github.com/someone/ardurbot",
      branch: "trunk",
      repointRemote: true,
    });
  ...
  it("keeps the repository URL in argv rather than in any compose input", () => {
    const repoUrl = "https://github.com/someone/ardurbot";
    const steps = composeUpdatePlan({
      strategy: "build",
      target,
      repoUrl,
...
describe("managed env assignments", () => {
  it("replaces a managed key in place and leaves everything else alone", () => {
    ...
    expect(upsertEnvAssignments(contents, { ARDURBOT_IMAGE_TAG: "v1.1.0" })).toBe(
      ["# deployment", "POSTGRES_PASSWORD=secret", "ARDURBOT_IMAGE_TAG=v1.1.0", ""].join("\n"),
    );
  });

  it("appends keys that are not present yet", () => {
    const result = upsertEnvAssignments("POSTGRES_PASSWORD=secret\n", {
      ARDURBOT_IMAGE_TAG: "v1.1.0",
  ...
  it("rewrites every duplicate managed assignment so the last value cannot win", () => {
    const result = upsertEnvAssignments(
      "ARDURBOT_IMAGE_TAG=stale\nA=1\nARDURBOT_IMAGE_TAG=still-stale\n",
      { ARDURBOT_IMAGE_TAG: "sha-0123456789abcdef0123456789abcdef01234567" },
    );
    expect(result.match(/ARDURBOT_IMAGE_TAG=sha-/g)).toHaveLength(2);
  ...
  it("refuses values that would inject a second assignment or a compose expression", () => {
    ...
    for (const value of ["v1\nEVIL=1", "v1 v2", "$(id)", "v1;rm -rf /", "${OTHER}"]) {
      expect(() => upsertEnvAssignments("", { ARDURBOT_IMAGE_TAG: value })).toThrow(/Refusing/);
    }
  });
