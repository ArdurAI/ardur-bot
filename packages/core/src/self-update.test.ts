...
describe("normalizeRepoUrl", () => {
  it("accepts the official repository and https forks", () => {
    ...
    expect(normalizeRepoUrl("https://git.example.com:8443/team/ardurbot.git")).toEqual({
      url: "https://git.example.com:8443/team/ardurbot.git",
    });
  });

  it("accepts ssh remotes in both spellings", () => {
    expect(normalizeRepoUrl("git@github.com:me/ardurbot.git")).toEqual({
      url: "git@github.com:me/ardurbot.git",
    });
    expect(normalizeRepoUrl("ssh://git@github.com/me/ardurbot.git")).toEqual({
      url: "ssh://git@github.com/me/ardurbot.git",
    });
  });
  ...
  it("refuses transports and shapes that are not a git remote", () => {
    for (const input of [
      "",
      "   ",
      "http://github.com/me/ardurbot",
      "git://github.com/me/ardurbot",
      "file:///etc/passwd",
      "/srv/ardurbot",
      "https://github.com",
      "https://github.com/onlyone",
      "https://github.com/me/../../etc",
      "https://user:secret@github.com/me/ardurbot",
      "https://token@github.com/me/ardurbot",
      "ssh://bad%0auser@github.com/me/ardurbot",
      "https://github.com/me/ardurbot?x=1",
      "https://github.com/me/ardurbot#frag",
      "https://github.com/me/rak azo",
      "--upload-pack=touch /tmp/pwned",
      "-o ProxyCommand=id",
      `https://github.com/me/ardurbot${"x".repeat(400)}`,
    ]) {
  ...
  it("rejects an embedded newline that could smuggle a second argument", () => {
    expect(normalizeRepoUrl("https://github.com/me/ardurbot\n--exec=id")).toHaveProperty("error");
  });
});
...
describe("updateSteps", () => {
  ...
  it("passes every value as its own argument so nothing reaches a shell", () => {
    const steps = updateSteps({
      remoteUrl: "git@github.com:me/ardurbot.git",
      branch: "release/1.2",
      targetCommit: "2".repeat(40),
      repointRemote: true,
    });
    expect(steps[0]).toEqual({
      ...
      command: "git",
      args: ["remote", "set-url", "origin", "git@github.com:me/ardurbot.git"],
    });
