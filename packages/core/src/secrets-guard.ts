...
const RUNTIME_SECRETS_ERROR =
  "Set BETTER_AUTH_SECRET and ENCRYPTION_KEY to long random strings before starting Ardur Bot outside local development or tests.";
...
export function isDevSecretAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  if (env.ARDURBOT_ALLOW_DEV_SECRETS === "1") return true;
...
export function resolveUpdaterToken(env: NodeJS.ProcessEnv = process.env): string {
  return resolveDedicatedSecret(env, {
    name: "ARDURBOT_UPDATER_TOKEN",
    conflicts: ["BETTER_AUTH_SECRET", "SANDBOX_SUPERVISOR_TOKEN", "SCREEN_PROXY_SECRET"],
    missingMessage: "Set ARDURBOT_UPDATER_TOKEN to a dedicated random updater credential.",
    conflictMessage:
      "ARDURBOT_UPDATER_TOKEN must differ from BETTER_AUTH_SECRET, SANDBOX_SUPERVISOR_TOKEN, and SCREEN_PROXY_SECRET.",
  });
}
 ...
 * through `@ardurbot/core`, and importing `node:crypto` here fails the production Vite build with
