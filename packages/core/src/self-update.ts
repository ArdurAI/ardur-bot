/** The repository a stock Ardur Bot deployment tracks. */
export const OFFICIAL_REPO_URL = "https://github.com/ardurai/ardur-bot";
...
/** `github.com/ardurai/ardur-bot` for every spelling of the same remote, for comparison only. */
...
export function updateSteps(input: UpdatePlanInput): UpdateStep[] {
  ...
  steps.push(
    ...
    },
    {
      ...
      command: "pnpm",
      args: ["--filter", "@ardurbot/db", "run", "generate"],
    },
    {
      id: "build",
      label: "Build the web app",
      command: "pnpm",
      args: ["--filter", "@ardurbot/web", "run", "build"],
    },
    {
      id: "migrate",
      label: "Apply database migrations",
      command: "pnpm",
      args: ["--filter", "@ardurbot/db", "run", "migrate"],
    },
  );
  return steps;
}
...
export const RESTART_SUPERVISOR_ENV = "ARDURBOT_UPDATE_RESTART_SUPERVISOR";
...
export function restartSupervisorAdvice(supervisor: RestartSupervisor): string {
  if (supervisor.kind !== "none") {
    return `Ardur Bot will exit after updating and ${supervisor.label} will start it on the new code.`;
  }
  return `No process supervisor was detected, so Ardur Bot will not exit on its own. Restart the API, worker, and web processes yourself, or set ${RESTART_SUPERVISOR_ENV} to the supervisor that restarts them (for example "docker" with restart: unless-stopped, or run under systemd with Restart=always).`;
}
