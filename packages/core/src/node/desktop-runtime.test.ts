    ...
    spawnSync(
      ...
      [
        ...
        [
          ...
          script
            .replaceAll("/tmp/ardurbot/desktop-assignments", root)
...
describe("shared Linux desktop lifecycle", () => {
  it("allocates live slots past 1000 bots, keeps assignments across callers, and rejects stale leases", () => {
    ...
    expect(f.ensure("bot-1000").stdout).toContain("ARDURBOT_DESKTOP=1000:view-bot-1000");
  ...
  it("reserves failed startup and teardown slots until a successful retry", () => {
    ...
    expect(f.ensure("c").stdout).toContain("ARDURBOT_DESKTOP=2:view-c");
    expect(f.release("a").status).toBe(0);
    expect(f.ensure("d").stdout).toContain("ARDURBOT_DESKTOP=0:view-d");
  });
  ...
  it("skips damaged assignments while reserving their live display markers", () => {
    ...
    expect(f.ensure("a").stdout).toContain("ARDURBOT_DESKTOP=1:view-a");
    expect(f.ensure("b").stdout).toContain("ARDURBOT_DESKTOP=2:view-b");
    expect(f.ensure("a").stdout).toContain("ARDURBOT_DESKTOP=1:view-a");
  });
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "pkill() { :; }; sleep() { :; }",
          `bash() { printf '%s\\n' "$5" >>${shellQuote(record)}; }`,
          resetDesktopRuntimeCommand(env).replaceAll("/tmp/ardurbot", f.root),
        ].join("\n"),
      ],
      { encoding: "utf8", timeout: 5000 },
    );
  ...
  it("stops only the selected VNC transport and keeps the shared gateway", () => {
    ...
    expect(
      patterns.some((pattern) =>
        pattern.test(
          "/usr/bin/x11vnc -display :21 -rfbport 0 -unixsock /tmp/ardurbot/sockets/view-21-token -forever",
        ),
      ),
    ).toBe(true);
  });
