...
description: "The signed-in product is a long-running API, a Graphile Worker, Postgres, and a computer provider (Docker supervisor, E2B, Daytona, or Box). It is not a static site. The marketing…"
source_path: "docs/self-host.md"
---

> [Source: docs/self-host.md](https://github.com/ArdurAI/ardur-bot/blob/__ARDUR_BOT_SOURCE_REF__/docs/self-host.md). Edit the source file, then run `python3 site/scripts/sync_docs.py` to refresh this page.
...
Pull Postgres and `ghcr.io/ardurai/ardur-bot/app` into any empty folder. No clone or image build.
...
[Self-host secrets checklist](/docs/self-host-secrets/). To customize the public URL, image tag, or
...
`ARDURBOT_AUTO_REVIEW_PROVIDER=jev` and `TYPESAFE_API_KEY`. Core still runs with neither.
...
`ARDURBOT_HOST` to its hostname (for example, `app.example.com`).
...
[restricted-network guide](/docs/self-host-restricted-network/) for mirror settings and local files.
...
| --- | --- | --- |
| `ARDURBOT_COMPUTER_MEMORY` | `2g` | `2g`, `1536m`, a byte count. Minimum `6m`, Docker's own floor. Also caps swap, so the ceiling holds. |
| `ARDURBOT_COMPUTER_CPUS` | `2` | Whole or fractional cores, e.g. `1.5` |
| `ARDURBOT_COMPUTER_PIDS_LIMIT` | `2048` | A positive integer |
...
Services set `service.name` (`ardurbot-api`, `ardurbot-worker`, `ardurbot-sandbox-supervisor`,
`ardurbot-updater`). A partial Axiom config logs a one-time warning and stays off. `AXIOM_EDGE` is a
...
ARDURBOT_LOCAL_MODELS_URL=http://127.0.0.1:11434/v1
ARDURBOT_LOCAL_CONTEXT_WINDOW=32768
ARDURBOT_LOCAL_MAX_TOKENS=4096
...
Public hosts and ordinary hostnames need `ARDURBOT_OPENAI_COMPAT_ALLOW_PUBLIC=1` and HTTPS.
...
`ARDURBOT_OPENAI_COMPATIBLE_VISION_MODELS=gpt4o-vision,llava`.
...
not available until verified migration lands; see [Fleet](/docs/fleet/).
...
For provider configuration and health checks, see the [provider setup guide](/docs/self-host-sandbox-providers/).
...
2. Clone the repository on the VM and create a root `.env` with production-only values. At minimum set
   ...
   `ARDURBOT_HOST`, and the three public origins. Set `ARDURBOT_DEPLOY_DIR` when the checkout is not at
   the supported Linux default, `/srv/ardurbot`. Use URL-safe random values for database credentials.
   If you enable the `updater` profile, also set a dedicated `ARDURBOT_UPDATER_TOKEN` (at least 32
...
WAKEUP_DRIVER=graphile
DATA_DIR=/data
# Absolute path of this checkout as the Docker daemon sees it. /srv/ardurbot is the Linux default;
# set this explicitly for every other layout. See "The deploy directory must be one path" below.
ARDURBOT_DEPLOY_DIR=/srv/ardurbot
ARDURBOT_IMAGE_TAG=local
# Optional: required only with `--profile updater`.
# ARDURBOT_UPDATER_TOKEN=replace-with-32-plus-character-updater-token
```
...
**Build, do not pull, for a first deployment.** `ARDURBOT_IMAGE_TAG` ships as `local`, a tag no
...
`ARDURBOT_IMAGE_TAG` to that release tag and running `pull` followed by `up -d --wait --pull never`.
...
`/usr/local/sbin/ardurbot-backup` and enable the supplied `ardurbot-backup.timer`. It creates a verified
Postgres custom-format dump plus an application-data archive under `/var/backups/ardurbot`, with mode
...
The scheduled backup uses `/srv/ardurbot` by default. For another deployment directory, set
`ARDURBOT_DEPLOY_DIR=/absolute/path/to/checkout` in a root-owned `/etc/ardurbot/backup.env`
...
For backups created by `scripts/backup.sh`, use an empty `ardurbot` database in the development
...
production snapshot's custom-format `ardurbot.dump` or `appdata.tgz`.
...
`ARDURBOT_IMAGE_TAG` (or rebuild `local`) and `up -d --wait --pull never`.

The updater sidecar has its own image and tag so an update never recreates the process performing
it. Move it deliberately by setting `ARDURBOT_UPDATER_IMAGE_TAG` to the full `sha-<commit>` tag, then
...
`GIT_SHA=$(git rev-parse HEAD)`, run `pnpm --filter @ardurbot/db migrate`, then restart API and worker.
...
content addressing can pin `ARDURBOT_IMAGE` outside the automatic updater to a verified digest.
...
`ARDURBOT_UPDATER_TOKEN` and explicitly start the profile:
...
`ARDURBOT_UPDATER_TOKEN`. Operator CLI upgrades above do not need it; the sidecar is for automated
...
- *Official repository:* resolves the newest stable release and its source commit with
  ...
  outgoing tag in `ARDURBOT_IMAGE_TAG_PREVIOUS`, explicitly pulls the new image, then runs
...
- *Fork (Advanced):* a fork has no published images, so the sidecar fast-forwards the checkout in
  `ARDURBOT_DEPLOY_DIR` and runs `up -d --build`. This builds on the server and takes minutes rather
...
`ARDURBOT_DEPLOY_DIR` is bind-mounted into the updater at the same path it is read from
(`${ARDURBOT_DEPLOY_DIR}:${ARDURBOT_DEPLOY_DIR}`), and that is load-bearing rather than tidy. Production
Compose defaults both sides to `/srv/ardurbot`; set the variable for any other layout. When the
updater runs `docker compose -p <project> --file $ARDURBOT_DEPLOY_DIR/infra/compose/docker-compose.prod.yml up -d`,
...
`ARDURBOT_COMPOSE_PROJECT_NAME`; the final fallback is `ardurbot-prod`. Without that propagation, a
...
`ARDURBOT_COMPOSE_FILE` takes a list, separated the way Compose's own `COMPOSE_FILE` is
...
```
ARDURBOT_COMPOSE_FILE=infra/compose/docker-compose.prod.yml:ops/compose/overlay.yml
```

Every entry is validated separately and must stay inside `ARDURBOT_DEPLOY_DIR`.

If the overlay adds a service built from the application image, name it in
`ARDURBOT_UPDATE_SERVICES` (comma separated) so it is pulled, recreated and rolled back with the
rest. Otherwise an update leaves that service running the previous code:

```
ARDURBOT_UPDATE_SERVICES=supervisor
```
...
- **Linux.** The daemon shares the host filesystem, so the checkout path is the answer:
  `/srv/ardurbot` is the default and supported production layout. Set `ARDURBOT_DEPLOY_DIR` explicitly
  when the checkout is elsewhere.
- **Docker Desktop (Windows/macOS).** The daemon runs in a VM that mounts your drive somewhere else.
  On Windows, `C:` appears at `/run/desktop/mnt/host/c`, so a checkout at `C:\Users\you\ardurbot` is
  `ARDURBOT_DEPLOY_DIR=/run/desktop/mnt/host/c/Users/you/ardurbot`. Host Git may use `core.autocrlf=true`; the updater ignores CR-only diffs so that does not block `/apply`. Verify the mount before deploying:

```bash
docker compose --env-file .env -f infra/compose/docker-compose.prod.yml \
  --profile updater run --rm updater git -C "$ARDURBOT_DEPLOY_DIR" log --oneline -1
```
...
- The process environment carries only updater settings (`ARDURBOT_UPDATER_TOKEN`, deploy path,
...
Enabling the `updater` profile requires `ARDURBOT_UPDATER_TOKEN` to be a dedicated random value (at
