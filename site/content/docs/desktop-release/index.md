---
title: "Desktop pre-releases"
description: "Desktop previews are unsigned. No Developer ID, notarization credentials, or Windows certificate"
source_path: "docs/desktop-release.md"
---

> [Source: docs/desktop-release.md](https://github.com/ArdurAI/ardur-bot/blob/__ARDUR_BOT_SOURCE_REF__/docs/desktop-release.md). Edit the source file, then run `python3 site/scripts/sync_docs.py` to refresh this page.
...
[performance evidence](/docs/performance/#evidence-index); only a dispatch with a waiver reason can
...
4. Wait for the pre-release assets. Download the DMG matching the Mac architecture, drag
   **Ardur Bot.app** to **Applications**, eject the DMG, and follow the unsigned-opening steps
   in the [README](/docs/readme/#install-a-desktop-preview). Open it, confirm the version in update
...
5. Verify a real installed build before deciding whether `main` should move. Create a bot and
   ...
   **Settings → Computers** shows this computer with its free memory, CPU and disk. See
   [where bots run](/docs/self-host-guide/#where-bots-run) for an app that already runs a Compose stack.
