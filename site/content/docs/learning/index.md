...
description: "Learning is disabled by default. Enabling it lets a configured model produce proposals after a"
source_path: "docs/learning.md"
---

> [Source: docs/learning.md](https://github.com/ArdurAI/ardur-bot/blob/__ARDUR_BOT_SOURCE_REF__/docs/learning.md). Edit the source file, then run `python3 site/scripts/sync_docs.py` to refresh this page.
...
acceptance criteria, labels and evidence remain visible until approval; the board it will use is
resolved server-side and is not shown here. A board-item
...
on the Board.” instead. Undo closes a new item with `Undone from Learning` only
while it is still open and unchanged; otherwise, whether someone else closed it or only edited it, it says
