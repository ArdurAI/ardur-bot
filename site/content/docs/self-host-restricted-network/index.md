---
title: "Self-hosting on restricted networks"
description: "Use a mirror you control or pre-copy files when GitHub or container registries are unreachable."
source_path: "docs/self-host-restricted-network.md"
---

> [Source: docs/self-host-restricted-network.md](https://github.com/ArdurAI/ardur-bot/blob/__ARDUR_BOT_SOURCE_REF__/docs/self-host-restricted-network.md). Edit the source file, then run `python3 site/scripts/sync_docs.py` to refresh this page.
...
| Cannot fetch Compose files | `ARDURBOT_DOWNLOAD_BASE`, `--local`, or `ARDURBOT_DOWNLOAD_SKIP_EXISTING=1` |
| Cannot pull app or computer images | `ARDURBOT_IMAGE`, `ARDURBOT_IMAGE_TAG`, `ARDURBOT_COMPUTER_IMAGE`, `ARDURBOT_COMPUTER_IMAGE_TAG` |
...
```bash
export ARDURBOT_INSTALLER_URL=https://example.com/mirror/ardurbot/infra/compose/install-images.sh
export ARDURBOT_DOWNLOAD_BASE=https://example.com/mirror/ardurbot/infra/compose
mkdir -p ardurbot && cd ardurbot &&
curl -fsSL -o install-images.sh "${ARDURBOT_INSTALLER_URL}" &&
bash install-images.sh --prepare-only
```

`ARDURBOT_INSTALLER_URL` selects the script for this curl command; `ARDURBOT_DOWNLOAD_BASE` selects
...
ARDURBOT_DOWNLOAD_SKIP_EXISTING=1 bash install-images.sh --prepare-only
```
...
```env
ARDURBOT_IMAGE=registry.example.com/mirror/ardurbot/app
ARDURBOT_IMAGE_TAG=edge
...
See the [base daemon configuration](https://github.com/ArdurAI/ardur-bot/blob/__ARDUR_BOT_SOURCE_REF__/infra/compose/docker-daemon.json). Docker Hub mirrors do not
...
mirror tags. After startup, follow the health checks in [the setup prompt](https://github.com/ArdurAI/ardur-bot/blob/__ARDUR_BOT_SOURCE_REF__/SETUP_PROMPT.md).

Once downloads work, continue with [published-image setup](/docs/self-host-guide/#published-images-no-checkout).
For hosts without external provider access, use local Docker computers and an operator-controlled
model endpoint; see the [self-hosting guide](/docs/self-host-guide/).
