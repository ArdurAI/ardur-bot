 ...
 * a host-wide memory and CPU event that takes every other bot and the Ardur Bot
...
export function computerResourceLimits() {
  const memoryBytes = parseMemoryBytes(
    ...
    envOrDefault("ARDURBOT_COMPUTER_MEMORY", DEFAULT_COMPUTER_MEMORY),
  );
  const nanoCpus = parseNanoCpus(
    "ARDURBOT_COMPUTER_CPUS",
    envOrDefault("ARDURBOT_COMPUTER_CPUS", DEFAULT_COMPUTER_CPUS),
  );
  const pidsLimit = parsePidsLimit(
    "ARDURBOT_COMPUTER_PIDS_LIMIT",
    envOrDefault("ARDURBOT_COMPUTER_PIDS_LIMIT", DEFAULT_COMPUTER_PIDS_LIMIT),
  );
  return {
...
export function containerCreateOptions(input: ComputerCreateInput) {
  ...
  return {
    ...
    Env: [
      ...
      "PIP_USER=1",
      ...(input.controlToken ? [`ARDURBOT_COMPUTER_CONTROL_TOKEN=${input.controlToken}`] : []),
    ],
    Labels: {
      "ardurbot.managed": "true",
      "ardurbot.botId": input.botId,
      "ardurbot.spaceId": input.spaceId,
    },
    ...
    HostConfig: {
      ...
      ...(input.homeVolume
        ...
        : { Binds: [`${input.homePath}:/home/ardurbot`], Mounts: undefined }),
...
export function containerNameFor(botId: string) {
  return `ardurbot-bot-${sanitizeIdentifier(botId)}`;
}
...
export function computerNetworkNameFor(botId: string) {
  ...
  return `ardurbot-computer-${sanitizeIdentifier(botId).slice(0, 32)}-${hash}`;
}
...
export function computerNetworkNamesForCleanup(botId: string) {
  ...
  return [
    ...
    `ardurbot-computer-${safe}`,
    `ardurbot-computer-${safe.slice(0, 32)}-${digest.slice(0, 8)}`,
  ];
}
