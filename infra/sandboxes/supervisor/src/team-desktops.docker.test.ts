...
it.skipIf(process.env.VERIFY_DOCKER_TEAM_SCREENS !== "1").each([false, true])(
  ...
  (managed) => {
    const name = `ardurbot-team-screens-test-${randomUUID()}`;
    const directory = mkdtempSync(path.join(tmpdir(), "ardurbot-team-screens-"));
    ...
    try {
      ...
      const commands: Record<string, string> = {
        ...
        seed: managed
          ? `python3 -c 'from pathlib import Path; d=Path("/tmp/ardurbot/desktop-assignments"); [(d / ("fixture-%s.slot" % i)).write_text(str(i) + "\\n") for i in range(1, 99)]'`
          : "true",
      ...
      docker(
        ...
        "none",
        "--shm-size=512m",
        process.env.ARDURBOT_COMPUTER_IMAGE ?? "ardurbot/computer:local",
      );
