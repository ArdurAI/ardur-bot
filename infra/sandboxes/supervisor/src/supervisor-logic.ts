...
} from "@ardurbot/core/node/desktop-runtime";
...
export { BROWSER_APPLICATIONS as DOCKER_BROWSER_ALIASES } from "@ardurbot/core/node/desktop-runtime";
...
export function hasComputerIdentity(
  ...
  const labeledSpaceId = labels?.["ardurbot.spaceId"] ?? labels?.["ardurbot.workspaceId"];
  return labels?.["ardurbot.botId"] === botId && labeledSpaceId === spaceId;
}
...
export function containerActionStep(
  ...
  if (action.kind === "key" || action.kind === "pointer" || action.kind === "clipboard") {
    ...
    argv = [
      "env",
      `DISPLAY=${display}`,
      ...(browserProfile ? [`ARDURBOT_BROWSER_PROFILE=${browserProfile}`] : []),
      "xdg-open",
      target,
    ];
  } else {
    ...
    argv = [
      "env",
      `DISPLAY=${display}`,
      ...(browser && browserProfile ? [`ARDURBOT_BROWSER_PROFILE=${browserProfile}`] : []),
      browser ? "ardurbot-browser" : action.application,
...
export function workspaceTarget(relative: string) {
  return relative ? path.posix.join("/home/ardurbot", relative) : "/home/ardurbot";
}
