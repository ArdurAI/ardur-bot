...
import { browserProfilePathForScreen } from "@ardurbot/core/node/desktop-runtime";
  ...
  (browserProfile) => {
    it.each([
      ...
      if (
        ...
        expect(step.argv[2]).toBe(`ARDURBOT_BROWSER_PROFILE=${browserProfile}`);
      }
...
describe("controller argv restrictions", () => {
  it.each([
    ["env", `DISPLAY=${display}`, `ARDURBOT_BROWSER_PROFILE=${profile}`],
    ["env", `DISPLAY=${display}`, "ARDURBOT_BROWSER_PROFILE=", "ardurbot-browser"],
    ["env", `DISPLAY=${display}`, `ARDURBOT_BROWSER_PROFILE=${profile}a`, "ardurbot-browser"],
    [
      "env",
      `DISPLAY=${display}`,
      `ARDURBOT_BROWSER_PROFILE=${profile.toUpperCase()}`,
      "ardurbot-browser",
    ],
    ["env", `DISPLAY=${display}`, `ARDURBOT_BROWSER_PROFILE=${profile}`, "xterm"],
    [
      "env",
      `DISPLAY=${display}`,
    ...
    ["env", `DISPLAY=${display}`, `ARDURBOT_BROWSER_PROFILE=${profile}`, "xdg-open"],
    [
      "env",
      `DISPLAY=${display}`,
      `ARDURBOT_BROWSER_PROFILE=${profile}`,
    ...
    ["env", "DISPLAY=:8", `ARDURBOT_BROWSER_PROFILE=${profile}`, "ardurbot-browser"],
