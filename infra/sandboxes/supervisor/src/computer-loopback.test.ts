...
describe("computer loopback provision lifecycle", () => {
  ...
  it.each([
    ...
    if (resumed) {
      ...
      expect(options.HostConfig.Binds).toEqual([`${homePath}:/home/ardurbot`]);
      expect(options.Env).toContainEqual(
        expect.stringMatching(/^ARDURBOT_COMPUTER_CONTROL_TOKEN=.+/),
      );
    }
  });
});
...
describe("space computer limit enforcement", () => {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        return [
          ...
          { Id: "c2", Labels: { "ardurbot.managed": "true", "ardurbot.spaceId": "space-1" } },
        ];
      },
    );
  ...
  it("resumes existing container even if space is at limit", async () => {
    ...
    const existing = {
      ...
      inspect: vi.fn().mockResolvedValue({
        ...
        Config: {
          ...
          Labels: {
            "ardurbot.managed": "true",
            "ardurbot.botId": "bot-existing",
            "ardurbot.spaceId": "space-1",
          },
        },
        State: { Running: true },
        HostConfig: {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        const labels = opts?.filters?.label ?? [];
        if (labels.some((l: string) => l === "ardurbot.botId=bot-existing")) {
          return [
            {
              Id: existing.id,
              Labels: {
  ...
  it("counts legacy workspaceId COMPUTER_IMAGE containers toward the limit", async () => {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        // Legacy managed computer: COMPUTER_IMAGE + workspaceId, no ardurbot.managed.
        return [
          {
            Id: "legacy",
            Image: COMPUTER_IMAGE,
            Labels: { "ardurbot.workspaceId": "space-1", "ardurbot.botId": "legacy-bot" },
          },
        ];
      },
    );
