...
loader = importlib.machinery.SourceFileLoader("page_browser", str(Path(__file__).with_name("ardurbot-page-browser")))
...
class PageBrowserTest(unittest.TestCase):
    def test_closed_stdin_cancels_helper_without_a_browser(self):
        with subprocess.Popen([sys.executable, str(Path(__file__).with_name("ardurbot-page-browser")), "snapshot", "{}"],
                              env={**os.environ, "ARDURBOT_BROWSER_WATCH_STDIN": "1", "ARDURBOT_CDP_PORT": "0"},
