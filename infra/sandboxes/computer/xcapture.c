...
void *ardurbot_xcapture_open(const char *display_name) {
...
int ardurbot_xcapture_copy(void *context, const unsigned char **rgb, int *width, int *height) {
...
int ardurbot_xcapture_png(void *context, const unsigned char **png_bytes, size_t *png_size,
    int *width, int *height) {
  XCapture *capture = context;
  if (!png_bytes || !png_size) return -1;
  const unsigned char *rgb = NULL;
  if (ardurbot_xcapture_copy(context, &rgb, width, height)) return -1;
...
int ardurbot_xcapture_damage(void *context, int *x, int *y, int *width, int *height) {
  XCapture *capture = context;
...
void ardurbot_xcapture_close(void *context) {
  XCapture *capture = context;
  if (!capture) return;
...
int ardurbot_xinput_argv(void *context, int argc, const char *const *argv) {
  XCapture *capture = context;
