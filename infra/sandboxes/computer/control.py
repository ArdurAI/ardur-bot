...
TOKEN = os.environ.get("ARDURBOT_COMPUTER_CONTROL_TOKEN", "")
...
class NativeCapture:
    ...
    def __init__(self, display):
        library = ctypes.CDLL("/usr/local/lib/libardurbot-xcapture.so")
        library.ardurbot_xcapture_open.argtypes = [ctypes.c_char_p]
        library.ardurbot_xcapture_open.restype = ctypes.c_void_p
        library.ardurbot_xcapture_png.argtypes = [
            ctypes.c_void_p,
        ...
        library.ardurbot_xcapture_png.restype = ctypes.c_int
        library.ardurbot_xcapture_damage.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
        ...
        library.ardurbot_xcapture_damage.restype = ctypes.c_int
        library.ardurbot_xinput_argv.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
        library.ardurbot_xinput_argv.restype = ctypes.c_int
        context = library.ardurbot_xcapture_open(display.encode("utf-8"))
        if not context:
    ...
    def copy(self):
        ...
        if self.library.ardurbot_xcapture_png(
        ...
        changed = self.library.ardurbot_xcapture_damage(
    ...
    def act(self, argv):
        ...
        return self.library.ardurbot_xinput_argv(self.context, len(argv), encoded)
...
def control_command_index(argv):
    ...
    return 3 if len(argv) > 2 and argv[2].startswith("ARDURBOT_BROWSER_PROFILE=") else 2
...
def allowed_control_argv(argv, display):
    ...
    command = argv[index]
    if index == 3:
        profile = argv[2].removeprefix("ARDURBOT_BROWSER_PROFILE=")
        if not re.fullmatch(r"/home/ardurbot/\.browser-profiles/chromium-bot-[a-f0-9]{32}", profile):
            return False
        if command not in ("xdg-open", "ardurbot-browser"):
            return False
    if command == "xdotool":
