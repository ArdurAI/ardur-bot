...
PROJECT_DIR="${ARDURBOT_DEPLOY_DIR:-/srv/ardurbot}"
[[ "${PROJECT_DIR}" == /* ]] || { echo "ARDURBOT_DEPLOY_DIR must be an absolute path" >&2; exit 1; }
COMPOSE_FILE="${PROJECT_DIR}/infra/compose/docker-compose.prod.yml"
ENV_FILE="${PROJECT_DIR}/.env"
BACKUP_ROOT="/var/backups/ardurbot"
...
"${compose[@]}" exec -T postgres pg_restore --list \
  < "${SNAPSHOT_DIR}/ardurbot.dump" >/dev/null
tar -tzf "${SNAPSHOT_DIR}/appdata.tgz" >/dev/null

sha256sum "${SNAPSHOT_DIR}/ardurbot.dump" "${SNAPSHOT_DIR}/appdata.tgz" \
...
echo "Verified Ardur Bot backup written to ${SNAPSHOT_DIR}"
