...
case "$DOWNLOAD_BASE" in
  https://*) ;;
  *)
    echo "Ardur Bot setup failed: ARDURBOT_DOWNLOAD_BASE must use https." >&2
    exit 1
    ;;
esac
...
if [[ "$prepare_only" == true ]]; then
  echo "Ardur Bot files are ready. Edit .env, then run: bash install-images.sh"
  exit 0
fi
...
echo "Ardur Bot is starting at http://127.0.0.1:5173"
