...
[[ "$offline_out" == *"Ardur Bot is starting"* ]] || fail "--offline did not start"
...
[[ "$old_out" != *"Ardur Bot setup failed:"* ]] || fail "old Compose --offline should not hard-fail: $old_out"
[[ "$old_out" == *"Ardur Bot is starting"* ]] || fail "old Compose --offline should continue: $old_out"
