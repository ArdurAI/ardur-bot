...
import { RECREATED_SERVICES } from "@ardurbot/core";
...
describe("the updater compose service", () => {
  ...
  it("is bind-mounted at the same path it has on the host", () => {
    const mount = (updater.volumes ?? []).find((volume) => volume.includes("ARDURBOT_DEPLOY_DIR"));
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    const deployDir = "${ARDURBOT_DEPLOY_DIR:-/srv/ardurbot}";
    ...
    expect(updater.environment?.ARDURBOT_DEPLOY_DIR).toBe(deployDir);
  ...
  it("pins its own image tag separately from the application image", () => {
    expect(updater.image).toContain("ARDURBOT_UPDATER_IMAGE_TAG");
    for (const service of RECREATED_SERVICES) {
  ...
  it("uses the official registry namespace and digest-pins third-party runtime images", () => {
    expect(updater.image).toContain("ghcr.io/ardurai/ardur-bot/updater");
    expect(compose.services.api?.image).toContain("ghcr.io/ardurai/ardur-bot/app");
  ...
  it("lets a deployment name its own Compose files and extra services", () => {
    ...
    expect(updater.environment?.ARDURBOT_COMPOSE_FILE).toBe(
      interpolated("ARDURBOT_COMPOSE_FILE", "infra/compose/docker-compose.prod.yml"),
    );
    expect(updater.environment?.ARDURBOT_UPDATE_SERVICES).toBe(
      interpolated("ARDURBOT_UPDATE_SERVICES"),
    );
  ...
  it("does not load the application env_file into the root-equivalent process", () => {
    ...
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    expect(updater.environment?.ARDURBOT_UPDATER_TOKEN).toBe("${ARDURBOT_UPDATER_TOKEN:-}");
  });
  ...
  it("does not let the api container reach the Docker socket to update itself", () => {
    ...
    expect(compose.services.api?.environment?.ARDURBOT_UPDATER_URL).toBe("http://updater:7092");
  });
