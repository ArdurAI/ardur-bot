...
import { type Logger, SERVICE_NAMES } from "@ardurbot/logging";
...
export function commandEnvironment(
  ...
  const deployDir = source.ARDURBOT_DEPLOY_DIR?.trim();
  const gitOwnership =
...
export function createUpdaterApp(
  ...
  async function writeEnvAssignments(assignments: Record<string, string>) {
    ...
    const temporary = `${config.envFile}.ardurbot-update-${randomUUID()}`;
  ...
  async function apply(request: { repoUrl: string; branch: string; official: boolean }) {
    ...
    if (decision.strategy === "build") {
      if (!checkout.present) {
        throw new UpdateRefused(
          "Building a fork needs the deployment's git checkout, and ARDURBOT_DEPLOY_DIR has no .git directory. Clone the fork to the deployment directory, or switch back to the official repository to use published images.",
        );
      }
