...
describe("source-checkout compose postgres hardening", () => {
  it("keeps postgres network-internal and requires POSTGRES_PASSWORD from .env", () => {
    ...
    expect(postgres?.environment?.POSTGRES_USER).toBe("${POSTGRES_USER:-ardurbot}");
    expect(postgres?.environment?.POSTGRES_DB).toBe("${POSTGRES_DB:-ardurbot}");
    for (const name of ["api", "worker"] as const) {
      expect(compose.services[name]?.environment?.DATABASE_URL).toBe(
        "postgres://${POSTGRES_USER:-ardurbot}:${POSTGRES_PASSWORD:?Set POSTGRES_PASSWORD in .env}@postgres:5432/${POSTGRES_DB:-ardurbot}",
      );
    }
  });
