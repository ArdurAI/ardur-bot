...
const originalEnv = `ARDURBOT_IMAGE_TAG=${oldTag}\nARDURBOT_IMAGE_TAG_PREVIOUS=v0.9.0\n`;
...
if (command === "docker") {
  ...
  call.imageTag = process.env.ARDURBOT_IMAGE_TAG;
  log();
...
async function deployment(options: { failures: Failure[]; branch?: string; composePath?: string }) {
  const deployDir = await mkdtemp(path.join(os.tmpdir(), "ardurbot-fork-recovery-"));
  ...
  const config = resolveUpdaterConfig({
    ARDURBOT_DEPLOY_DIR: deployDir,
    ARDURBOT_UPDATER_TOKEN: token,
    ARDURBOT_COMPOSE_FILE: options.composePath,
  ...
  const compose = (revision: string) =>
    `services:\n  api:\n    image: example/app:\${ARDURBOT_IMAGE_TAG}\n    env_file: ./service.env\n    command: ${revision}\n`;
  await Promise.all([
    mkdir(bin),
...
describe.skipIf(process.platform === "win32")("fork recovery through fake executables", () => {
  ...
  it("keeps the new checkout and image pin after a successful update", async () => {
    ...
    expect(await readFile(fixture.config.envFile, "utf8")).toContain(
      `ARDURBOT_IMAGE_TAG=local-${targetCommit}\n`,
    );
  });
});
