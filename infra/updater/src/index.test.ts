...
import type { ServerUpdateRun } from "@ardurbot/contracts";
...
const app = createUpdaterApp(
  resolveUpdaterConfig({
    ARDURBOT_DEPLOY_DIR: "/ardurbot-updater-tests-no-such-directory",
    ARDURBOT_UPDATER_TOKEN: token,
  }),
);
...
async function deployment(env = "ARDURBOT_IMAGE_TAG=v1.0.0\nARDURBOT_IMAGE_TAG_PREVIOUS=v0.9.0\n") {
  const deployDir = await mkdtemp(path.join(os.tmpdir(), "ardurbot-updater-test-"));
  temporaryDirectories.push(deployDir);
  await writeFile(path.join(deployDir, ".env"), env);
  return {
    deployDir,
    config: resolveUpdaterConfig({ ARDURBOT_DEPLOY_DIR: deployDir, ARDURBOT_UPDATER_TOKEN: token }),
  };
}
...
describe("updater HTTP surface", () => {
  ...
  it("refuses a fork build when the deployment has no checkout to build from", async () => {
    ...
    const response = await createUpdaterApp(fixture.config).request("/apply", {
      method: "POST",
      headers: authorized,
      body: JSON.stringify({ repoUrl: "https://github.com/someone/ardurbot", branch: "main" }),
    });
    expect(response.status).toBe(400);
    const payload = (await response.json()) as { error: string };
  ...
  it("refuses a rollback when no previous tag was recorded", async () => {
    const fixture = await deployment("ARDURBOT_IMAGE_TAG=v1.0.0\n");
...
describe("updater orchestration", () => {
  ...
  it("restores the prior image after a recreate fails health checks", async () => {
    ...
    expect(composeUp[0]?.env?.ARDURBOT_IMAGE_TAG).toBe(`sha-${targetCommit}`);
    expect(composeUp[1]?.env?.ARDURBOT_IMAGE_TAG).toBe("v1.0.0");
  ...
  it("resets a fork checkout when recreate fails after the fast-forward", async () => {
    const fixture = await deployment("ARDURBOT_IMAGE_TAG=local\n");
    await mkdir(path.join(fixture.deployDir, ".git"));
    const calls: string[][] = [];
    let upCalls = 0;
    const run: UpdaterCommandRunner = async (command, args) => {
  ...
  it("refuses to replace a symlinked deployment environment", async () => {
    ...
    await writeFile(target, "ARDURBOT_IMAGE_TAG=v1.0.0\n");
    await rm(envFile);
    ...
    expect(await readFile(target, "utf8")).toBe("ARDURBOT_IMAGE_TAG=v1.0.0\n");
  });
...
describe("child process environment", () => {
  it("declares the deployment directory safe so git works on a non-root checkout", () => {
    ...
    const env = commandEnvironment({ ARDURBOT_DEPLOY_DIR: "/srv/ardurbot" });
    expect(env.GIT_CONFIG_COUNT).toBe("1");
    expect(env.GIT_CONFIG_KEY_0).toBe("safe.directory");
    expect(env.GIT_CONFIG_VALUE_0).toBe("/srv/ardurbot");
  });

  it("keeps the exemption scoped to the deployment directory, whoever calls it", () => {
    const env = commandEnvironment(
      { ARDURBOT_DEPLOY_DIR: "/opt/ardurbot" },
      { GIT_CONFIG_VALUE_0: "*", GIT_CONFIG_COUNT: "9" },
    );
    expect(env.GIT_CONFIG_VALUE_0).toBe("/opt/ardurbot");
    expect(env.GIT_CONFIG_COUNT).toBe("1");
  });
