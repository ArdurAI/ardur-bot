...
export function resolveUpdaterConfig(env: NodeJS.ProcessEnv): UpdaterConfig {
  const deployDir = env.ARDURBOT_DEPLOY_DIR?.trim() ?? "";
  if (deployDir === "" || !path.posix.isAbsolute(deployDir)) {
    throw new Error("Set ARDURBOT_DEPLOY_DIR to the absolute path of the deployment directory.");
  }
  const image = env.ARDURBOT_IMAGE?.trim() || OFFICIAL_SERVER_IMAGE;
  if (!isValidImageName(image))
    throw new Error(`ARDURBOT_IMAGE is not a usable image name: ${image}`);
  ...
  const composeFiles = (env.ARDURBOT_COMPOSE_FILE?.trim() || DEFAULT_COMPOSE_FILE)
    .split(separator)
    .map((entry) => entry.trim())
    .filter((entry) => entry !== "");
  if (composeFiles.length === 0) {
    throw new Error("ARDURBOT_COMPOSE_FILE must name at least one Compose file.");
  }
  for (const composeFile of composeFiles) {
    if (path.posix.isAbsolute(composeFile) || composeFile.split("/").includes("..")) {
      throw new Error("ARDURBOT_COMPOSE_FILE must list paths inside the deployment directory.");
    }
  }
  ...
  const extraServices = (env.ARDURBOT_UPDATE_SERVICES ?? "")
    .split(",")
    .map((entry) => entry.trim())
    .filter((entry) => entry !== "");
  for (const service of extraServices) {
    if (!SERVICE_NAME.test(service)) {
      throw new Error(`ARDURBOT_UPDATE_SERVICES contains an unusable service name: ${service}`);
    }
    ...
    if (service === UPDATER_SERVICE) {
      throw new Error(
        "ARDURBOT_UPDATE_SERVICES must not contain the updater: it cannot recreate itself mid-update.",
      );
    }
  }
  ...
  const port = Number(env.ARDURBOT_UPDATER_PORT ?? DEFAULT_UPDATER_PORT);
  if (!Number.isInteger(port) || port <= 0 || port > 65_535) {
    throw new Error("ARDURBOT_UPDATER_PORT is not a port number.");
  }
  return {
    deployDir,
    ...
    host: env.ARDURBOT_UPDATER_HOST?.trim() || "127.0.0.1",
    port,
  };
}
