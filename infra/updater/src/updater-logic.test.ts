import { DEFAULT_COMPOSE_PROJECT_NAME, isLocalImageTag } from "@ardurbot/core";
...
const base = {
  ...
  ARDURBOT_UPDATER_TOKEN: "fake-review-updater-token-000000000000",
} as const;
...
describe("resolveUpdaterConfig", () => {
  it("derives the compose file, env file, and defaults from the deployment directory", () => {
    ...
    expect(config).toMatchObject({
      deployDir: "/srv/ardurbot",
      composeFiles: [`/srv/ardurbot/${DEFAULT_COMPOSE_FILE}`],
      updateServices: ["api", "worker", "web"],
      envFile: "/srv/ardurbot/.env",
      projectName: DEFAULT_COMPOSE_PROJECT_NAME,
      token: base.ARDURBOT_UPDATER_TOKEN,
  ...
  it("uses the Compose-injected project name so -p matches the running stack", () => {
    ...
    expect(
      resolveUpdaterConfig({ ...base, ARDURBOT_COMPOSE_PROJECT_NAME: "manual-stack" }).projectName,
    ).toBe("manual-stack");
  });
  ...
  it("refuses a compose path that escapes the deployment directory", () => {
    for (const composeFile of ["/etc/compose.yml", "../../etc/compose.yml", "a/../../b.yml"]) {
      expect(() => resolveUpdaterConfig({ ...base, ARDURBOT_COMPOSE_FILE: composeFile })).toThrow(
        /ARDURBOT_COMPOSE_FILE/,
      );
    }
  });
  ...
  it("honours COMPOSE_PATH_SEPARATOR the way Compose does", () => {
    expect(
      resolveUpdaterConfig({
        ...
        ARDURBOT_COMPOSE_FILE: "a.yml,b.yml",
      }).composeFiles,
    ).toEqual(["/srv/ardurbot/a.yml", "/srv/ardurbot/b.yml"]);
  });
  ...
  it("checks every entry in a list, not just the first", () => {
    expect(() =>
      resolveUpdaterConfig({
        ...base,
        ARDURBOT_COMPOSE_FILE: "infra/compose/docker-compose.prod.yml:../../etc/compose.yml",
      }),
    ).toThrow(/ARDURBOT_COMPOSE_FILE/);
    expect(() =>
      resolveUpdaterConfig({
        ...base,
        ARDURBOT_COMPOSE_FILE: "infra/compose/docker-compose.prod.yml:/etc/compose.yml",
  ...
  it("refuses a Compose file list that names nothing", () => {
    expect(() => resolveUpdaterConfig({ ...base, ARDURBOT_COMPOSE_FILE: ":  :" })).toThrow(
      /ARDURBOT_COMPOSE_FILE/,
    );
  });
  ...
  it("cannot drop a built-in service, however ARDURBOT_UPDATE_SERVICES is written", () => {
    const services = resolveUpdaterConfig({
      ...base,
      ARDURBOT_UPDATE_SERVICES: "web,supervisor",
    }).updateServices;
  ...
  it("refuses to recreate the updater, which would kill the run mid-flight", () => {
    expect(() =>
      resolveUpdaterConfig({ ...base, ARDURBOT_UPDATE_SERVICES: "supervisor,updater" }),
    ).toThrow(/updater/);
  });
  ...
  it("refuses a service name it would not hand to compose as one argument", () => {
    for (const service of ["--build", "a b", "-f", "api;rm"]) {
      expect(() =>
        resolveUpdaterConfig({ ...base, ARDURBOT_UPDATE_SERVICES: `supervisor,${service}` }),
      ).toThrow(/ARDURBOT_UPDATE_SERVICES/);
    }
  });

  it("refuses an image name it would not be willing to hand to compose", () => {
    expect(() => resolveUpdaterConfig({ ...base, ARDURBOT_IMAGE: "Bad Name" })).toThrow(
      /ARDURBOT_IMAGE/,
    );
    expect(resolveUpdaterConfig({ ...base, ARDURBOT_IMAGE: "ghcr.io/me/app" }).image).toBe(
      "ghcr.io/me/app",
    );
  });

  it("refuses a port that is not a port", () => {
    expect(() => resolveUpdaterConfig({ ...base, ARDURBOT_UPDATER_PORT: "0" })).toThrow(/port/);
    expect(() => resolveUpdaterConfig({ ...base, ARDURBOT_UPDATER_PORT: "seven" })).toThrow(/port/);
  });
});

describe("readEnvAssignment", () => {
  it("reads the last assignment, ignoring comments and blank lines", () => {
    const contents = ["# ARDURBOT_IMAGE_TAG=commented", "", "A=1", "A=2"].join("\n");
    expect(readEnvAssignment(contents, "A")).toBe("2");
    expect(readEnvAssignment(contents, "ARDURBOT_IMAGE_TAG")).toBeNull();
  });
  ...
  it("does not match a key that merely shares a prefix", () => {
    expect(readEnvAssignment("ARDURBOT_IMAGE_TAG_PREVIOUS=v1", "ARDURBOT_IMAGE_TAG")).toBeNull();
  });
});

describe("readTagState", () => {
  it("reads the pinned tag and the rollback tag", () => {
    const contents = "ARDURBOT_IMAGE_TAG=v1.1.0\nARDURBOT_IMAGE_TAG_PREVIOUS=v1.0.0\n";
  ...
  it("ignores values in the file that are not usable tags", () => {
    const contents = "ARDURBOT_IMAGE_TAG=-rm\nARDURBOT_IMAGE_TAG_PREVIOUS=$(id)\n";
