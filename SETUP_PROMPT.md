# Set up Ardur Bot with a coding agent
...
```text
Set up Ardur Bot from published GHCR images and leave the web UI running.
...
1. Which directory should contain the Ardur Bot folder (or use the current directory)?
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during Ardur Bot onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
...
2. Download and inspect this installer (do not clone the repository):
   https://raw.githubusercontent.com/ArdurAI/ardur-bot/main/infra/compose/install-images.sh
   If that host is unreachable, use a mirror URL (e.g. set `ARDURBOT_INSTALLER_URL` to
   `https://example.com/mirror/ardurbot/infra/compose/install-images.sh` and curl that instead).
3. Run `bash install-images.sh --prepare-only`. It downloads the Compose and environment example
   ...
   Optional: set `ARDURBOT_DOWNLOAD_BASE` to a generic mirror of `infra/compose`, and/or use
   `--local` / `ARDURBOT_DOWNLOAD_SKIP_EXISTING=1` when Compose files are already present locally.
...
```text
Set up Ardur Bot locally and leave it running in a usable state.

Repository: https://github.com/ArdurAI/ardur-bot.git
...
1. Should you clone into the current directory, or what parent directory should contain `ardurbot`? If you are already inside a Ardur Bot checkout, offer to use it without recloning.
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during Ardur Bot onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
   - Defer model setup and verify infrastructure only. Make clear that bots cannot answer until a model is connected.
...
- If I requested Electron, leave the web stack running and then launch `pnpm --filter @ardurbot/desktop dev`. Verify the shell loads the same app. Let me make the Docker-versus-This-Mac choice because This Mac grants bots access under my OS account.
