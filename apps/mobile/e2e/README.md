...
expects a running Ardur Bot stack and deliberately stays out of ordinary pull-request CI.

## Prerequisites

1. Install the Maestro CLI and start an Android emulator or iOS simulator.
2. Start Ardur Bot's database, API, worker, and sandbox supervisor. The computer portion requires a
...
4. Build/install the native app with an API URL that the emulator can reach. For the standard local
   ...
   ```sh
   EXPO_PUBLIC_API_URL=http://10.0.2.2:3100 pnpm --filter @ardurbot/mobile android
   ```
...
```sh
pnpm --filter @ardurbot/mobile test:e2e -- \
  -e ARDURBOT_E2E_EMAIL=mobile-smoke@example.test \
  -e ARDURBOT_E2E_PASSWORD='replace-with-the-disposable-password' \
  -e ARDURBOT_E2E_BOT_NAME=MaestroSmoke-001 \
  -e ARDURBOT_E2E_MESSAGE=mobile-smoke-message
```
