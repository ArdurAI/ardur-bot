import type { LearningProposal, ProposalEvidence, SpaceLearningConfig } from "@ardurbot/contracts";
...
export default function Learning() {
  ...
  const load = useCallback(async () => {
    const [inbox, config] = await Promise.all([loadLearning(botId), loadLearningSettings()]);
            ...
            <View key={proposal.id} style={styles.card}>
              ...
              {open === proposal.id ? (
                <View>
                  ...
                  {proposal.provenance ? (
                    <Text style={styles.secondary}>
                      {proposal.provenance.runId} · {proposal.provenance.originatingPin?.modelId} ·{" "}
                      {proposal.provenance.reviewerPin.modelId} ·{" "}
                      {proposal.provenance.reviewerPin.effort} · {proposal.provenance.policyVersion}
                    </Text>
                  ) : null}
                  ...
                  {proposal.status === "applied" ? (
                    <Text style={styles.secondary}>{t("no observations yet")}</Text>
                  ) : null}
                  {conflict ? (
                    <View>
