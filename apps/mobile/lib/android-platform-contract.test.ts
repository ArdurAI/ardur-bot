...
describe("Android mobile platform contract", () => {
  ...
  it("requests live-update promotion and exposes its Android settings", () => {
    const nativeRoot = resolve(
      mobileRoot,
      "modules/ardurbot-notifications/android/src/main/java/ai/ardur/bot/notifications",
    );
    const service = readFileSync(resolve(nativeRoot, "ArdurBotNotificationService.kt"), "utf8");
    const module = readFileSync(resolve(nativeRoot, "ArdurBotNotificationsModule.kt"), "utf8");
    ...
    expect(module).toContain("ArdurBotNotificationService.clearSession(context)");
    ...
    expect(service).toContain('connection.setRequestProperty("x-ardurbot-space-id", spaceId)');
    ...
    expect(service).toContain('"ardurbot://group-thread?groupId=');
    expect(service).toMatch(/&spaceId=\$\{Uri\.encode\(run\.spaceId\)\}/);
    expect(service).toContain('putString("ardurbot.spaceId", run.spaceId)');
  ...
  it("shows live updates only for working runs and ties the pill to a real bot", () => {
    const service = readFileSync(
      resolve(
        mobileRoot,
        "modules/ardurbot-notifications/android/src/main/java/ai/ardur/bot/notifications/ArdurBotNotificationService.kt",
      ),
      "utf8",
    );
    expect(service).toContain(
    ...
    expect(service).toContain('putString("ardurbot.botId", run.botId)');
