...
const nativeNotifications =
  Platform.OS === "android"
    ? requireNativeModule<NativeNotifications>("ArdurBotNotifications")
    : null;
...
export function notificationTargetsThread(
  ...
  const dataThreadId = data.threadId ?? data["ardurbot.threadId"];
  if (target.threadId && dataThreadId) return dataThreadId === target.threadId;
  return Boolean(
    target.botId && (data.botId === target.botId || data["ardurbot.botId"] === target.botId),
  );
}
