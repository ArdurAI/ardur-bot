...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://ardurbot.example.com:8443/api/")).toEqual({
      ok: true,
      url: "https://ardurbot.example.com:8443",
    });
...
describe("display and warnings", () => {
  ...
  it("shows host and non-default port", () => {
    expect(displayApiHost("https://ardurbot.example.com")).toBe("ardurbot.example.com");
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://ardurbot.example.com")).toBe(true);
  });
});
