...
const ENDPOINT_KEY = "ardurbot.api_base";
const SPACE_KEY = "ardurbot.space_id";
const SPACE_ROLLBACK_KEY = "ardurbot.space_rollback";
  ...
  try {
    await withAbort(
      fetch(`${currentApiBase()}/api/auth/sign-out`, {
        method: "POST",
        headers: { "content-type": "application/json", origin: "ardurbot://", ...headers },
...
export async function rpc<T>(
  ...
  const requestSpaceId = requestHeaders["x-ardurbot-space-id"];
  try {
    let res: Response;
    try {
