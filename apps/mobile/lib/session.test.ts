...
describe("mobile session storage", () => {
  ...
  it("stores and clears only the session token key", async () => {
    ...
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith("ardurbot.session_token", "secret-token");
  ...
  it("overwrites the token when SecureStore delete fails", async () => {
    ...
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith("ardurbot.session_token", "");
  });
  ...
  it("returns an empty token when secure storage is empty or unavailable", async () => {
    ...
    expect(SecureStore.getItemAsync).toHaveBeenNthCalledWith(1, "ardurbot.session_token");
    expect(SecureStore.getItemAsync).toHaveBeenNthCalledWith(2, "ardurbot.session_token");
  });
