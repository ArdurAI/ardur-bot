...
class ArdurBotNotificationService : Service() {
  ...
  private fun post(run: RunRecord, copy: NotificationCopy) {
    ...
    val notification = builder(copy.channel)
      .setSmallIcon(R.drawable.ic_ardurbot_notification)
      .setContentTitle(copy.title)
      .setContentText(copy.body)
      .setStyle(Notification.BigTextStyle().bigText(copy.body))
      .setContentIntent(openApp(run))
      .addExtras(Bundle().apply {
        putString("ardurbot.spaceId", run.spaceId)
        putString("ardurbot.botId", run.botId)
        putString("ardurbot.threadId", run.threadId)
      })
      .setAutoCancel(true)
  ...
  private fun liveStatusIcon(run: RunRecord, avatarStyle: String): Icon {
    if (avatarStyle != "organic") {
      return Icon.createWithResource(this, R.drawable.ic_ardurbot_notification)
    }
  ...
  private fun openApp(run: RunRecord? = null): PendingIntent {
    val intent = if (run == null) {
      packageManager.getLaunchIntentForPackage(packageName) ?: Intent(Intent.ACTION_VIEW, Uri.parse("ardurbot://"))
    } else {
      val destination = if (run.groupId != null) {
        "ardurbot://group-thread?groupId=${Uri.encode(run.groupId)}&name=${Uri.encode(run.groupName.orEmpty())}&spaceId=${Uri.encode(run.spaceId)}"
      } else {
        "ardurbot://thread?botId=${Uri.encode(run.botId)}&name=${Uri.encode(run.botName)}&spaceId=${Uri.encode(run.spaceId)}"
      }
      Intent(
        Intent.ACTION_VIEW,
  ...
  companion object {
    ...
    private const val ACTION_THREAD_CHANGED = "ai.ardur.bot.notifications.THREAD_CHANGED"
    private const val PROMOTED_ONGOING_EXTRA = "android.requestPromotedOngoing"
    private const val POLL_INTERVAL_MS = 8_000L
    private const val STATE_PREFERENCES = "com.ardurbot.notification_state"
    ...
    fun start(context: Context) {
      val intent = Intent(context, ArdurBotNotificationService::class.java)
      context.startService(intent)
    }

    fun stop(context: Context) {
      context.stopService(Intent(context, ArdurBotNotificationService::class.java))
    }
    ...
    fun setOpenThread(context: Context, botId: String?, threadId: String?) {
      ...
      context.startService(
        Intent(context, ArdurBotNotificationService::class.java).setAction(ACTION_THREAD_CHANGED),
      )
    }
...
private object Channels {
  const val LIVE = "ardurbot_live"
  const val MESSAGES = "ardurbot_messages"
  const val SCHEDULED = "ardurbot_scheduled"
  const val ATTENTION = "ardurbot_attention"
}
...
private fun rpc(
  ...
  return try {
    ...
    connection.setRequestProperty("Origin", "ardurbot://")
    connection.setRequestProperty("Authorization", "Bearer $token")
    if (spaceId.isNotBlank()) {
      connection.setRequestProperty("x-ardurbot-space-id", spaceId)
    }
