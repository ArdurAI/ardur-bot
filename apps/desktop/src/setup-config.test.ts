...
describe("server address normalization", () => {
  it("assumes http locally and https for a bare public host", () => {
    ...
    expect(normalizeServerUrl("ardurbot.example.com")).toBe("https://ardurbot.example.com");
  });

  it("keeps an explicit secure scheme and port but stores only the origin", () => {
    expect(normalizeServerUrl("https://ardurbot.example.com")).toBe("https://ardurbot.example.com");
    expect(normalizeServerUrl("https://ardurbot.example.com:8443/team")).toBe(
      "https://ardurbot.example.com:8443",
    );
  });
  ...
  it("rejects cleartext public servers but permits private-network development", () => {
    expect(normalizeServerUrl("http://ardurbot.example.com")).toBeNull();
  ...
  it("rejects embedded credentials rather than writing them to disk", () => {
    expect(normalizeServerUrl("https://user:secret@ardurbot.example.com")).toBeNull();
  });
});
...
describe("managed local open URL", () => {
  ...
  it("rejects a different loopback origin even when both are local", () => {
    ...
    expect(managedLocalOpenUrl("https://ardurbot.example.com", DEFAULT_LOCAL_WEB_URL)).toBeNull();
  });
});

describe("desktop stack token transport", () => {
  it("allows HTTPS and loopback HTTP only", () => {
    expect(maySendDesktopStackToken("https://ardurbot.example.com")).toBe(true);
    ...
    expect(maySendDesktopStackToken("http://ardurbot.local:5173")).toBe(false);
  });
});

describe("saved setup", () => {
  it("round-trips through the on-disk format", () => {
    const setup = { mode: "existing", serverUrl: "https://ardurbot.example.com" } as const;
...
describe("startup target", () => {
  const saved = { mode: "existing", serverUrl: "https://ardurbot.example.com" } as const;
  ...
  it("opens the saved instance on later launches", () => {
    expect(resolveStartupTarget({ saved })).toEqual({
      kind: "app",
      url: "https://ardurbot.example.com",
      source: "saved",
    });
  });

  it("lets ARDURBOT_WEB_URL point the shell anywhere without touching saved setup", () => {
  ...
  it("ignores an empty ARDURBOT_WEB_URL", () => {
...
describe("bundled renderer eligibility", () => {
  it("stands in for http(s) origins only", () => {
    ...
    expect(servesBundledRenderer("https://ardurbot.example.com")).toBe(true);
...
describe("remote-content isolation", () => {
  it("uses a stable, opaque session partition per server origin", () => {
    ...
    expect(first).toMatch(/^persist:ardurbot-[a-f0-9]{24}$/);
...
describe("Ardur Bot health response", () => {
  it("requires the public RPC health contract", () => {
    ...
    expect(isArdurBotHealth({ ok: true, version: "0.1.0" })).toBe(false);
  });
});
