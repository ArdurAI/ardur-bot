...
describe("stack locations", () => {
  it("keeps the compose project under user data", () => {
    expect(stackDir("/data/ardurbot")).toBe(path.join("/data/ardurbot", "stack"));
  });
...
describe("ensureStackEnv", () => {
  let dir: string;
  beforeEach(async () => {
    dir = await mkdtemp(path.join(tmpdir(), "ardurbot-stack-env-"));
  });
  afterEach(async () => {
...
describe("stack identity", () => {
  let dir: string;
  beforeEach(async () => {
    dir = await mkdtemp(path.join(tmpdir(), "ardurbot-stack-token-"));
  });
  afterEach(async () => {
    await rm(dir, { recursive: true, force: true });
  });
...
describe("LocalStackController", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), "ardurbot-stack-"));
    calls = [];
    phases = [];
  });
  afterEach(async () => {
  ...
  it("keeps lifecycle commands off the standalone project despite environment overrides", async () => {
    ...
    for (const call of commands) {
      expect(call.args.slice(5, 7)).toEqual(["--project-name", "ardurbot-desktop"]);
  ...
  it("retries port conflicts on a new origin and uses it for auth, health, and saved launches", async () => {
    ...
    expect(
      calls.filter((call) => call.args[7] === "up").map((call) => call.env.ARDURBOT_WEB_PORT),
    ).toEqual(["5173", "45174"]);
