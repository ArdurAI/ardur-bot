...
import type { ArdurBotDesktopOAuthCallback } from "@ardurbot/contracts";
...
export async function openBrowserAuth(
  ...
  options: {
    ...
    onCallback: (callback: ArdurBotDesktopOAuthCallback) => void;
    onClose?: () => void;
  },
): Promise<void> {
      ...
      for (const host of hosts) {
        ...
        const server = createServer({ maxHeaderSize: 8192 }, (request, response) => {
          ...
          if (
            ...
            response.writeHead(400).end("Sign-in callback rejected. Return to Ardur Bot to retry.");
            return;
          }
          consumed = true;
          response.end("You can close this tab and return to Ardur Bot.", close);
