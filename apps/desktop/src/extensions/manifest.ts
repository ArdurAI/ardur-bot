...
export interface McpbManifest {
  ...
  server: {
    ...
    entry_point: string;
    mcp_config: ServerLaunch & { platform_overrides: Record<string, Partial<ServerLaunch>> };
  };
...
export function manifestText(value: unknown, max = 4096): string {
  if (
    typeof value !== "string" ||
    !value.trim() ||
    value.length > max ||
...
function launch(value: unknown, partial = false): Partial<ServerLaunch> {
...
function validateValue(field: UserConfigField, value: unknown): ConfigValue {
  if (field.multiple) {
...
function userFields(value: unknown): Record<string, UserConfigField> {
  const entries = Object.entries(record(value ?? {}));
...
export function parseMcpbManifest(value: unknown, platform = process.platform): McpbManifest {
  const data = record(value);
  ...
  return {
    ...
    ...(data.icon !== undefined && typeof data.icon === "string" && !data.icon.includes(":")
      ...
      : {}),
    server: {
      type: server.type as McpbManifest["server"]["type"],
      entry_point: bundlePath(manifestText(server.entry_point)),
      mcp_config: { ...(launch(config) as ServerLaunch), platform_overrides: overrides },
    },
...
export function resolveMcpbLaunch(
  manifest: McpbManifest,
  input: {
    directory: string;
  ...
  const vars: Record<string, ConfigValue> = {
    ...input.variables,
...
export function configurationFields(manifest: McpbManifest, values: Record<string, ConfigValue>) {
