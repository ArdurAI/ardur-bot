...
const componentKeys = ["skills", "commands", "mcpServers", "outputStyles"] as const;
...
function componentPaths(value: unknown): string[] | undefined {
  return value === undefined
    ? undefined
    : (Array.isArray(value) ? textList(value) : [manifestText(value)]).map(componentPath);
}
function stringMap(value: unknown, header = false): Record<string, string> {
...
export function parsePluginServers(value: unknown): Record<string, PluginServer> {
...
export function parseMarketplace(value: unknown) {
  const data = record(value);
  ...
  const plugins = data.plugins.map((value): MarketplaceEntry => {
...
export function pluginFiles(files: BundleFile[], source: string): BundleFile[] {
...
function markdownFiles(files: BundleFile[], paths: string[], skills: boolean) {
  return files
    .filter((file) =>
...
export function planPlugin(files: BundleFile[], entry?: MarketplaceEntry) {
  ...
  const addServers = (value: PluginManifest["mcpServers"]) => {
...
export function pluginInstallFiles(
  files: BundleFile[],
