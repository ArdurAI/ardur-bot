...
beforeEach(async () => {
  userData = await mkdtemp(path.join(tmpdir(), "ardurbot-setup-"));
});
...
describe("setup store", () => {
  ...
  it("keeps the chosen instance across launches", async () => {
    await writeSetup(userData, { mode: "existing", serverUrl: "https://ardurbot.example.com" });
    await expect(readSetup(userData)).resolves.toEqual({
      mode: "existing",
      serverUrl: "https://ardurbot.example.com",
    });
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await writeFile(
        target,
        JSON.stringify({ mode: "existing", serverUrl: "https://ardurbot.example.com" }),
        "utf8",
      );
