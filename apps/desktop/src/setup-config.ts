...
import type { DesktopSetup, DesktopStackProbeResponse } from "@ardurbot/contracts";
...
/**
 * Accepts what a person would actually type ("localhost:5173", "ardurbot.example.com")
 * and returns a canonical http(s) origin, or null when the input can never
 * securely address a Ardur Bot server.
 */
export function normalizeServerUrl(input: string): string | null {
  ...
  // Ardur Bot serves its renderer, RPC, and auth routes from one origin. Keeping a
 ...
 * `ARDURBOT_WEB_URL` still wins over saved configuration so test and performance
...
/** Each Ardur Bot origin gets its own persistent cookie and storage partition. */
export function sessionPartitionForServerUrl(targetUrl: string): string | null {
  try {
    ...
    return `persist:ardurbot-${digest}`;
  } catch {
    return null;
  }
}
...
export function isArdurBotHealth(value: unknown): boolean {
 ...
 * metadata endpoints. Cleartext HTTP to them is never a legitimate Ardur Bot
