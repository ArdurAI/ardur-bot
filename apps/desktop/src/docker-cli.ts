...
import type { DesktopSetupLink } from "@ardurbot/contracts";
...
/**
 * `ARDURBOT_DOCKER_BINARY` is a test hook and the only candidate when set, so a
...
export function resolveDockerBinary(
  ...
  const override = env.ARDURBOT_DOCKER_BINARY?.trim();
