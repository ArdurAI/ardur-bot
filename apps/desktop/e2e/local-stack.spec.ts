...
test.beforeAll(async () => {
  server = createServer(async (request, response) => {
    ...
    if (request.url === "/api/desktop-settings/rpc/integrationSetup/get") {
      const expected = await readFile(path.join(userData, "stack", ".desktop-stack-token"), "utf8");
      if (
        request.headers["x-ardurbot-local-settings-token"] !== expected.trim() ||
...
/**
 * Stands in for the docker CLI: records `cwd | ARDURBOT_IMAGE_TAG | argv` for every call and
...
async function writeFakeDocker(mode: FakeDockerMode) {
  ...
  let up = '  up) echo "Container ardurbot-web-1 Started"; exit 0 ;;';
  if (mode === "pool-exhausted") {
    ...
    up = `  up) if [ ! -f '${path.join(userData, "port-conflict")}' ]; then touch '${path.join(userData, "port-conflict")}'; echo "port is already allocated" >&2; exit 1; fi; echo "Container ardurbot-web-1 Started"; exit 0 ;;`;
  }
  const lines = [
    "#!/bin/sh",
    `printf '%s | %s | %s\\n' "$PWD" "$ARDURBOT_IMAGE_TAG" "$*" >> '${fakeDockerLog()}'`,
    'case "$1 $2" in',
...
async function launch(mode: FakeDockerMode | "missing") {
  const env: NodeJS.ProcessEnv = { ...process.env, ARDURBOT_PERFORMANCE_USER_DATA: userData };
  ...
  return electron.launch({
    ...
    env: {
      ...env,
      ARDURBOT_DOCKER_BINARY:
        mode === "missing" ? "/nonexistent/docker" : await writeFakeDocker(mode),
      ARDURBOT_LOCAL_WEB_URL: serverUrl,
      ARDURBOT_IMAGE_TAG: IMAGE_TAG,
    },
  });
}
...
test("the native settings menu opens an isolated logged-out settings capability", async () => {
  ...
  const denied = await main.evaluate(async () => {
    try {
      await window.ardurbotDesktop?.localSettings?.request(
  ...
  const result = await settings.evaluate(() =>
    window.ardurbotDesktop?.localSettings?.request(
      "/api/desktop-settings/rpc/integrationSetup/get",
      "{}",
    ),
  );
