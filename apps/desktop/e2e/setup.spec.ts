...
const APP_MARKER = "Existing Ardur Bot instance ready";
...
test("first run asks whether to use a local or existing instance", async () => {
  app = await launch();
  if (process.env.ARDURBOT_E2E_EXECUTABLE) {
    expect(await app.evaluate(({ app }) => app.isPackaged)).toBe(true);
  }
  const setup = await app.firstWindow();

  await expect(setup.getByRole("heading", { name: "Welcome to Ardur Bot" })).toBeVisible();
...
test("connecting to an existing instance verifies, saves, and opens it", async () => {
  ...
  await expect(setup.locator("#status")).toHaveText(`Ardur Bot answered at ${serverUrl}.`);
...
test("a session-pending shell skeleton is not accepted as a ready app", async () => {
  const skeletonHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ardur Bot</title></head>
...
for (const { name, surface } of [
  ...
  test(`a post-session ${name} mount is accepted`, async () => {
    const readyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ardur Bot</title>
</head>
...
test("a shell mount before workspace bootstrap is not accepted", async () => {
  const preBootstrapHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ardur Bot</title></head>
...
test("a session-ready marker without a route surface is not accepted", async () => {
  const emptyReadyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ardur Bot</title></head>
...
test("a generic web page is not accepted as a Ardur Bot server", async () => {
  const plain = createServer((_request, response) => {
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end("<!doctype html><p>not Ardur Bot</p>");
  });
  ...
  try {
    ...
    await expect(setup.locator("#status")).toHaveText(
      "That address did not respond like a Ardur Bot server.",
    );
    await expect(async () => {
      await expect(readFile(path.join(userData, "setup.json"), "utf8")).rejects.toThrow();
    }).toPass();
...
test("the native application menu can reopen setup without exposing setup IPC to the server", async () => {
  app = await launch({ ARDURBOT_WEB_URL: serverUrl });
  const appWindow = await app.firstWindow();
  await expect(appWindow.getByText(APP_MARKER)).toBeVisible();

  const setupPromise = app.waitForEvent("window");
  await app.evaluate(({ Menu }) => {
    const item = Menu.getApplicationMenu()?.getMenuItemById("change-ardurbot-server");
...
test("servers on the same host but different ports do not share login cookies", async () => {
  const first = createServer((_request, response) => {
    response.writeHead(200, {
      ...
      "set-cookie": "ardurbot_session=fake-one; Path=/; SameSite=Lax",
    });
  ...
  try {
    app = await launch({ ARDURBOT_WEB_URL: `http://127.0.0.1:${firstAddress.port}` });
    ...
    app = await launch({ ARDURBOT_WEB_URL: `http://127.0.0.1:${secondAddress.port}` });
...
test("setup IPC is not reachable from the connected app window", async () => {
  ...
  const exposed = await appWindow.evaluate(() =>
    Object.keys((window as typeof window & { ardurbotSetup?: unknown }).ardurbotSetup ?? {}),
  );
