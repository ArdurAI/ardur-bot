...
describe("marketing site middleware", () => {
  it("negotiates Markdown on the canonical page URL", async () => {
    const response = middleware(
      new Request("https://github.com/ArdurAI/ardur-bot/", {
    ...
    await expect(response.text()).resolves.toContain("# Ardur Bot");
  });

  it("gives generic agent fetches a recoverable Markdown 404", async () => {
    const response = middleware(
      new Request("https://github.com/ArdurAI/ardur-bot/does-not-exist", {
        headers: { accept: "*/*" },
      }),
    );
    ...
    await expect(response.text()).resolves.toContain(
      "https://github.com/ArdurAI/ardur-bot/sitemap-index.xml",
    );
  });
