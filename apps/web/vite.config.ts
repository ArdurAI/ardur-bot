...
import type { DesktopStackProbeResponse } from "@ardurbot/contracts";
import {
  safeScreenProxyResponseHeaders,
  stripSensitiveHandshakeHeaders,
} from "@ardurbot/core/node/screen-proxy-response";
...
export default defineConfig(({ mode }) => {
  ...
  const performanceAssetDelayMs = Number(process.env.ARDURBOT_PERFORMANCE_ASSET_DELAY_MS ?? 0);
  const desktopStackToken =
    process.env.ARDURBOT_DESKTOP_STACK_TOKEN ?? rootEnv.ARDURBOT_DESKTOP_STACK_TOKEN ?? "";
  const imageTag = process.env.ARDURBOT_IMAGE_TAG ?? rootEnv.ARDURBOT_IMAGE_TAG ?? "edge";
  return {
    plugins: [
      react(),
      ...
      tailwindcss(),
      {
        name: "ardurbot-desktop-stack-probe",
      ...
      {
        name: "ardurbot-performance-asset-delay",
        configurePreviewServer(server) {
          if (!Number.isFinite(performanceAssetDelayMs) || performanceAssetDelayMs <= 0) return;
          server.middlewares.use((req, _res, next) => {
            const pathname = req.url?.split("?", 1)[0] ?? "/";
            if (
              ["/api", "/rpc", "/device", "/local/device-listener", "/local/system", "/novnc"].some(
    ...
    server: {
      ...
      proxy: {
        ...
        "/local/system": { target: api, changeOrigin: true },
      },
    },
    preview: {
      host: "0.0.0.0",
