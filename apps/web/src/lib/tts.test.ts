...
function stubSelectedSpace(id: string) {
  const store = new Map<string, string>([["ardurbot:space-id", id]]);
  const localStorage = {
  ...
  return (next: string) => store.set("ardurbot:space-id", next);
}
...
describe("Speaker", () => {
  ...
  it("keeps speak requests in the space where playback started", async () => {
    ...
    expect(headers.get("x-ardurbot-space-id")).toBe("space-support");
