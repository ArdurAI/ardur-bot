...
test("agent-attached files appear as downloadable cards", async ({ page }, testInfo) => {
  ...
  await signup(page, `artifact-card-${stamp}@ardurbot.test`, "password12", "Artifact Card");
...
test("agent-attached Markdown opens a rendered preview and can be downloaded", async ({
  ...
  const stamp = Date.now();
  await signup(page, `markdown-preview-${stamp}@ardurbot.test`, "password12", "Markdown Preview");
  await completeOnboarding(page);
