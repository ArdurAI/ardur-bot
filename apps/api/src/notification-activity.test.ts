...
it("marks a close whose bot can no longer use the board apart from five failed tries", async () => {
  const findMany = vi.fn(async () => [
    {
      id: "denied",
      title: "A board item filed by a bot could not be closed.",
      changes: ["close-denied"],
      createdAt: new Date("2026-09-25T01:00:00Z"),
      follow: { workspaceId: "board", itemId: "item", workspace: { spaceId: "space" } },
      workspaceId: null,
      itemId: null,
  ...
  const [denied, tried] = (
    await notificationActivity(prisma, { userId: "owner", spaceId: "space" } as Actor)
  ).activities;
  expect(denied?.board).toEqual({
    spaceId: "space",
    workspaceId: "board",
    itemId: "item",
    closeFailed: true,
