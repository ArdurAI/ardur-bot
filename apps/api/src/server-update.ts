...
import { ServerUpdateRunSchema } from "@ardurbot/contracts";
import {
...
export interface UpdaterProxyConfig {
  ...
  /** Current `ARDURBOT_IMAGE_TAG` when known; selects compose pull vs rebuild commands. */
...
export async function readServerUpdateStatus(
  ...
  const imageTagHint = config.imageTag?.trim() || process.env.ARDURBOT_IMAGE_TAG?.trim() || null;
