...
describe("server update install kind", () => {
  async function tempRoot(withGit: boolean) {
    const root = await mkdtemp(path.join(tmpdir(), "ardurbot-update-"));
    roots.push(root);
  ...
  it("detects sidecar when health and authenticated state succeed", async () => {
    ...
    const fetchImpl = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (href.endsWith("/state")) {
        return new Response(
          JSON.stringify({
            image: "ghcr.io/ardurai/ardur-bot/app",
            ...
            checkout: {
              ...
              branch: "main",
              remoteUrl: "https://github.com/ardurai/ardur-bot",
              dirty: false,
              dirtyPaths: [],
            },
          }),
...
describe("sidecar proxy auth and no-git-apply", () => {
  ...
  it("sends the bearer token to the sidecar and never returns it", async () => {
    ...
    const root = await mkdtemp(path.join(tmpdir(), "ardurbot-proxy-"));
    roots.push(root);
